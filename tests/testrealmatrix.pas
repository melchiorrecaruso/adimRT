{ TestRealMatrix.pas - Real matrix (TRMatrix) tests for ADimMath.

  Part of the modular ADimMath test suite.  Registers itself with
  TestFramework; the main program runs it automatically.

  To add a new test case here: add statements inside an existing Test*
  procedure, or write a new Test* procedure and call it from Run below.

  @author  Melchiorre Caruso
}
unit TestRealMatrix;

{$mode objfpc}{$H+}{$J-}
{$modeswitch advancedrecords}
{$modeswitch typehelpers}

interface

implementation

uses
  Math, SysUtils, StrUtils, ADimMath, TestFramework;

procedure TestTRMatrix;
var
  A, B, C_mat, Res: TRMatrix;
  v: TRVector;
  eigs: TArrayOfDouble;
  i, j: integer;

  // A = [[1,2,3],[4,5,6],[7,8,10]]
  procedure LoadA;
  begin A.Init(TArrayOfDouble.Create(1,2,3, 4,5,6, 7,8,10)); end;

  // B = [[2,0,1],[1,3,0],[0,1,2]]
  procedure LoadB;
  begin B.Init(TArrayOfDouble.Create(2,0,1, 1,3,0, 0,1,2)); end;

begin
  Section('TRMatrix - Init / New / Clone');

  A.Init(3);
  Check('Init(3) order = 3', A.Order = 3);
  Check('Init(3) all zero', A.IsNull);

  LoadA;
  Check('Init with data [1..10]: a[0,0]=1', Math.SameValue(A[0,0],1, EPS));
  Check('Init with data [1..10]: a[2,2]=10',Math.SameValue(A[2,2],10,EPS));

  C_mat := A.Clone;
  Check('Clone: same order', C_mat.Order = A.Order);
  Check('Clone: same data',  C_mat.SameValue(A));
  C_mat[0,0] := 999;
  Check('Clone: independent storage', not Math.SameValue(C_mat[0,0], A[0,0], EPS));

  Section('TRMatrix - Identity / Null / Diagonalize');

  A.Init(3);
  Res := A.Identity;
  Check('Identity[0,0]=1', Math.SameValue(Res[0,0],1,EPS));
  Check('Identity[0,1]=0', Math.SameValue(Res[0,1],0,EPS));
  Check('Identity[1,1]=1', Math.SameValue(Res[1,1],1,EPS));

  Res := A.Null;
  Check('Null is zero', Res.IsNull);

  Section('TRMatrix - Determinant / Trace / Rank / Norm');

  LoadA;
  CheckNear('det(A) = -3',   A.Determinant, -3.0,              EPS);
  LoadB;
  CheckNear('det(B) = 13',   B.Determinant, 13.0,              EPS);
  LoadA;
  Check('rank(A) = 3',       A.Rank = 3);
  CheckNear('trace(A) = 16', A.Trace,       16.0,              EPS);
  CheckNear('norm(A) frob',  A.Norm,        17.4355957741627,  EPS);

  // Rank-deficient matrix [[1,2,3],[2,4,6],[3,6,9]]
  C_mat.Init(TArrayOfDouble.Create(1,2,3, 2,4,6, 3,6,9));
  Check('rank([[1,2,3],[2,4,6],[3,6,9]]) = 1', C_mat.Rank = 1);

  // Zero matrix
  C_mat.Init(3);
  Check('rank(zeros) = 0', C_mat.Rank = 0);

  Section('TRMatrix - Add / Sub / Mul / ScalarMul / Div / Transpose');

  LoadA; LoadB;
  Res := A + B;
  Check('A+B [0,0]=3',  Math.SameValue(Res[0,0],3,  EPS));
  Check('A+B [1,1]=8',  Math.SameValue(Res[1,1],8,  EPS));
  Check('A+B [2,2]=12', Math.SameValue(Res[2,2],12, EPS));

  Res := A - B;
  Check('A-B [0,0]=-1', Math.SameValue(Res[0,0],-1, EPS));
  Check('A-B [2,2]=8',  Math.SameValue(Res[2,2],8,  EPS));

  // A*B = [[4,9,7],[13,21,16],[22,34,27]]
  Res := A * B;
  Check('A*B [0,0]=4',  Math.SameValue(Res[0,0],4,  EPS));
  Check('A*B [0,1]=9',  Math.SameValue(Res[0,1],9,  EPS));
  Check('A*B [1,0]=13', Math.SameValue(Res[1,0],13, EPS));
  Check('A*B [2,1]=34', Math.SameValue(Res[2,1],34, EPS));
  Check('A*B [2,2]=27', Math.SameValue(Res[2,2],27, EPS));

  Res := 2.0 * A;
  Check('2*A [0,0]=2',  Math.SameValue(Res[0,0],2,  EPS));
  Check('2*A [2,2]=20', Math.SameValue(Res[2,2],20, EPS));

  Res := A * 2.0;
  Check('A*2 [0,0]=2',  Math.SameValue(Res[0,0],2,  EPS));

  Res := A / 2.0;
  Check('A/2 [0,0]=0.5',Math.SameValue(Res[0,0],0.5,EPS));
  Check('A/2 [2,2]=5',  Math.SameValue(Res[2,2],5,  EPS));

  // Transpose: A^T [0,1] = A[1,0] = 4
  Res := A.Transpose;
  Check('A^T [0,1]=4', Math.SameValue(Res[0,1],4,   EPS));
  Check('A^T [1,0]=2', Math.SameValue(Res[1,0],2,   EPS));
  Check('A^T [2,0]=3', Math.SameValue(Res[2,0],3,   EPS));

  Section('TRMatrix - Reciprocal (inverse)');

  // inv(A): computed by numpy, verified analytically
  // inv(A) ~= [[-2/3, -4/3, 1],[-2/3, 11/3, -2],[1, -2, 1]]
  Res := A.Inverse;
  CheckNear('inv(A)[0,0] ~= -0.6667', Res[0,0], -2/3,  EPS);
  CheckNear('inv(A)[0,1] ~= -1.3333', Res[0,1], -4/3,  EPS);
  CheckNear('inv(A)[0,2] ~=  1',      Res[0,2],  1.0,  EPS);
  CheckNear('inv(A)[1,1] ~=  3.6667', Res[1,1],  11/3, EPS);
  CheckNear('inv(A)[2,0] ~=  1',      Res[2,0],  1.0,  EPS);

  // Verify A * inv(A) ~= I
  C_mat := A * Res;
  Check('A*inv(A)[0,0]~=1', Math.SameValue(C_mat[0,0],1,EPS));
  Check('A*inv(A)[0,1]~=0', Math.SameValue(C_mat[0,1],0,EPS));
  Check('A*inv(A)[1,1]~=1', Math.SameValue(C_mat[1,1],1,EPS));
  Check('A*inv(A)[2,2]~=1', Math.SameValue(C_mat[2,2],1,EPS));

  // 2x2 case
  A.Init(TArrayOfDouble.Create(3,1, 2,4));
  CheckNear('det([[3,1],[2,4]]) = 10', A.Determinant, 10.0, EPS);
  Res := A.Inverse;
  CheckNear('inv(2x2)[0,0]=0.4',  Res[0,0],  0.4,  EPS);
  CheckNear('inv(2x2)[0,1]=-0.1', Res[0,1], -0.1,  EPS);
  CheckNear('inv(2x2)[1,0]=-0.2', Res[1,0], -0.2,  EPS);
  CheckNear('inv(2x2)[1,1]=0.3',  Res[1,1],  0.3,  EPS);

  Section('TRMatrix - RowReduction');

  // [[2,1,-1],[4,3,1],[2,2,3]] has full rank -> RREF = I
  A.Init(TArrayOfDouble.Create(2,1,-1, 4,3,1, 2,2,3));
  Res := A.RowReduction;
  Check('RREF[0,0]=1', Math.SameValue(Res[0,0],1,EPS));
  Check('RREF[0,1]=0', Math.SameValue(Res[0,1],0,EPS));
  Check('RREF[0,2]=0', Math.SameValue(Res[0,2],0,EPS));
  Check('RREF[1,0]=0', Math.SameValue(Res[1,0],0,EPS));
  Check('RREF[1,1]=1', Math.SameValue(Res[1,1],1,EPS));
  Check('RREF[1,2]=0', Math.SameValue(Res[1,2],0,EPS));
  Check('RREF[2,0]=0', Math.SameValue(Res[2,0],0,EPS));
  Check('RREF[2,1]=0', Math.SameValue(Res[2,1],0,EPS));
  Check('RREF[2,2]=1', Math.SameValue(Res[2,2],1,EPS));

  Section('TRMatrix - Eigenvalues');

  // Symmetric 3x3 S = [[4,2,1],[2,5,3],[1,3,6]]
  // Eigenvalues (sorted): 1.9213469419616895, 3.730159123688258, 9.348493934350051
  A.Init(TArrayOfDouble.Create(4,2,1, 2,5,3, 1,3,6));
  eigs := RealEigenvalues(A.Eigenvalues);
  SortAscArray(eigs);
  CheckNear('S3 eig[0] ~= 1.9213', eigs[0], 1.9213469419616895, 1e-6);
  CheckNear('S3 eig[1] ~= 3.7302', eigs[1], 3.730159123688258,  1e-6);
  CheckNear('S3 eig[2] ~= 9.3485', eigs[2], 9.348493934350051,  1e-6);

  // Symmetric 4x4: [[5,1,0,2],[1,4,1,0],[0,1,3,1],[2,0,1,6]]
  // Eigenvalues: 1.81216339, 3.78382333, 4.54837849, 7.85563479
  A.Init(TArrayOfDouble.Create(5,1,0,2, 1,4,1,0, 0,1,3,1, 2,0,1,6));
  eigs := RealEigenvalues(A.Eigenvalues);
  SortAscArray(eigs);
  CheckNear('S4 eig[0] ~= 1.8122', eigs[0], 1.81216339, 1e-5);
  CheckNear('S4 eig[1] ~= 3.7838', eigs[1], 3.78382333, 1e-5);
  CheckNear('S4 eig[2] ~= 4.5484', eigs[2], 4.54837849, 1e-5);
  CheckNear('S4 eig[3] ~= 7.8556', eigs[3], 7.85563479, 1e-5);

  // 1x1 trivial
  A.Init(TArrayOfDouble.Create(7));
  eigs := RealEigenvalues(A.Eigenvalues);
  CheckNear('1x1 eigenvalue = 7', eigs[0], 7.0, EPS);

  // 2x2 symmetric [[3,1],[1,3]] eigenvalues 2 and 4
  A.Init(TArrayOfDouble.Create(3,1, 1,3));
  eigs := RealEigenvalues(A.Eigenvalues);
  SortAscArray(eigs);
  CheckNear('2x2 symm eig[0]=2', eigs[0], 2.0, 1e-6);
  CheckNear('2x2 symm eig[1]=4', eigs[1], 4.0, 1e-6);

  // Diagonal matrix: eigenvalues = diagonal entries
  A.Init(TArrayOfDouble.Create(3,0,0, 0,1,0, 0,0,5));
  eigs := RealEigenvalues(A.Eigenvalues);
  SortAscArray(eigs);
  CheckNear('diag(3,1,5) eig[0]=1', eigs[0], 1.0, 1e-6);
  CheckNear('diag(3,1,5) eig[1]=3', eigs[1], 3.0, 1e-6);
  CheckNear('diag(3,1,5) eig[2]=5', eigs[2], 5.0, 1e-6);

  Section('TRMatrix - IsUnitary (orthogonal)');

  // Identity is orthogonal
  A.Init(3);
  Res := A.Identity;
  Check('I3 IsUnitary=true',  Res.IsUnitary);

  // Rotation matrix around Z by 45deg
  A.Init(3);
  A[0,0] :=  cos(Pi/4); A[0,1] := -sin(Pi/4); A[0,2] := 0;
  A[1,0] :=  sin(Pi/4); A[1,1] :=  cos(Pi/4); A[1,2] := 0;
  A[2,0] :=  0;          A[2,1] :=  0;          A[2,2] := 1;
  Check('Rz(45deg) IsUnitary=true',  A.IsUnitary);

  // A (non-orthogonal) is not unitary
  LoadA;
  Check('A IsUnitary=false', not A.IsUnitary);

  Section('TRMatrix - SameValue / = / <>');

  LoadA;
  C_mat := A.Clone;
  Check('A = Clone(A)',     A = C_mat);
  Check('A.SameValue(A)',   A.SameValue(C_mat));
  C_mat[0,0] := 0;
  Check('A <> modified',    A <> C_mat);

  Section('TRMatrix - Swap');

  A.Init(TArrayOfDouble.Create(1,2, 3,4));
  A.Swap(0,1);
  Check('Swap rows: [0,0]=3', Math.SameValue(A[0,0],3,EPS));
  Check('Swap rows: [1,0]=1', Math.SameValue(A[1,0],1,EPS));
end;

procedure TestRealMatrices;
var
  M3, M4, G3, H4, Inv, Prod, Id: TRMatrix;
  S5, D5: TRMatrix;
  ev: TArrayOfDouble;
  i, j: integer;

  procedure SortAsc(var A: TArrayOfDouble);
  var p, q: integer; t: double;
  begin
    for p := 0 to High(A)-1 do
      for q := p+1 to High(A) do
        if A[p] > A[q] then begin t:=A[p]; A[p]:=A[q]; A[q]:=t; end;
  end;

begin
  // M3 = tridiagonal SPD [[2,-1,0],[-1,2,-1],[0,-1,2]]
  M3.Init(TArrayOfDouble.Create(2,-1,0, -1,2,-1, 0,-1,2));
  // M4 SPD-ish
  M4.Init(TArrayOfDouble.Create(4,1,2,0.5, 1,3,0,1, 2,0,5,1, 0.5,1,1,4));
  // G3 general non-symmetric
  G3.Init(TArrayOfDouble.Create(3,2,-1, 2,-2,4, -1,0.5,-1));
  // H4 Hilbert 4x4 (ill-conditioned)
  H4.Init(TArrayOfDouble.Create(
    1, 1/2, 1/3, 1/4,
    1/2, 1/3, 1/4, 1/5,
    1/3, 1/4, 1/5, 1/6,
    1/4, 1/5, 1/6, 1/7));

  // -- Determinant --
  BeginCategory('Real determinant');
  CmpR('det(M3)', M3.Determinant, 4.0);
  CmpR('det(M4)', M4.Determinant, 148.25000000000003);
  CmpR('det(G3)', G3.Determinant, -3.0000000000000036);
  CmpR('det(H4)', H4.Determinant, 1.6534391534392967e-07);

  // -- Trace --
  BeginCategory('Real trace');
  CmpR('trace(M3)', M3.Trace, 6.0);
  CmpR('trace(M4)', M4.Trace, 16.0);
  CmpR('trace(G3)', G3.Trace, 0.0);
  CmpR('trace(H4)', H4.Trace, 1.676190476190476);

  // -- Frobenius norm --
  BeginCategory('Real Frobenius norm');
  CmpR('norm(M3)', M3.Norm, 4.0);
  CmpR('norm(M4)', M4.Norm, 8.972179222463181);
  CmpR('norm(G3)', G3.Norm, 6.34428877022476);
  CmpR('norm(H4)', H4.Norm, 1.5097340998183073);

  // -- Inverse (M3, well-conditioned) --
  BeginCategory('Real inverse (M3)');
  Inv := M3.Inverse;
  CmpR('inv(M3)[0,0]', Inv[0,0], 0.75);
  CmpR('inv(M3)[0,1]', Inv[0,1], 0.5);
  CmpR('inv(M3)[0,2]', Inv[0,2], 0.24999999999999994);
  CmpR('inv(M3)[1,1]', Inv[1,1], 1.0);
  CmpR('inv(M3)[2,2]', Inv[2,2], 0.7499999999999999);
  // Verify M3 * inv = I
  Prod := M3 * Inv;
  Id   := M3.Identity;
  for i := 0 to 2 do
    for j := 0 to 2 do
      CmpR(Format('M3*inv-I[%d,%d]',[i,j]), Prod[i,j], Id[i,j]);

  // -- Inverse (M4) --
  BeginCategory('Real inverse (M4)');
  Inv := M4.Inverse;
  CmpR('inv(M4)[0,0]', Inv[0,0],  0.35075885328836426);
  CmpR('inv(M4)[0,1]', Inv[0,1], -0.12478920741989882);
  CmpR('inv(M4)[1,1]', Inv[1,1],  0.40978077571669475);
  CmpR('inv(M4)[2,2]', Inv[2,2],  0.2715008431703204);
  CmpR('inv(M4)[3,3]', Inv[3,3],  0.2900505902192243);
  Prod := M4 * Inv;
  Id   := M4.Identity;
  for i := 0 to 3 do
    for j := 0 to 3 do
      CmpR(Format('M4*inv-I[%d,%d]',[i,j]), Prod[i,j], Id[i,j]);

  // -- Inverse (H4, ill-conditioned - stress test) --
  BeginCategory('Real inverse (Hilbert4, ill-cond)');
  Inv := H4.Inverse;
  CmpR('inv(H4)[0,0]', Inv[0,0],  15.999999999999346);
  CmpR('inv(H4)[1,1]', Inv[1,1],  1199.9999999999045);
  CmpR('inv(H4)[2,2]', Inv[2,2],  6479.99999999942);
  CmpR('inv(H4)[3,3]', Inv[3,3],  2799.999999999749);
  // Reconstruction error H4*inv ~= I (looser by nature of conditioning)
  Prod := H4 * Inv;
  Id   := H4.Identity;
  for i := 0 to 3 do
    for j := 0 to 3 do
      CmpR(Format('H4*inv-I[%d,%d]',[i,j]), Prod[i,j], Id[i,j]);

  // -- Eigenvalues (symmetric matrices) --
  BeginCategory('Real eigenvalues (symmetric)');
  ev := RealEigenvalues(M3.Eigenvalues); SortAsc(ev);
  CmpR('eig(M3)[0]', ev[0], 0.585786437626905);
  CmpR('eig(M3)[1]', ev[1], 1.9999999999999998);
  CmpR('eig(M3)[2]', ev[2], 3.414213562373095);

  ev := RealEigenvalues(M4.Eigenvalues); SortAsc(ev);
  CmpR('eig(M4)[0]', ev[0], 1.6144288690811686);
  CmpR('eig(M4)[1]', ev[1], 3.166099049437209);
  CmpR('eig(M4)[2]', ev[2], 4.039519599886666);
  CmpR('eig(M4)[3]', ev[3], 7.179952481594955);

  ev := RealEigenvalues(H4.Eigenvalues); SortAsc(ev);
  CmpR('eig(H4)[0]', ev[0], 9.670230402260876e-05);
  CmpR('eig(H4)[1]', ev[1], 0.006738273605760613);
  CmpR('eig(H4)[2]', ev[2], 0.16914122022145006);
  CmpR('eig(H4)[3]', ev[3], 1.5002142800592426);

  // -- Sum of eigenvalues = trace (consistency) --
  BeginCategory('Real eig-trace consistency');
  ev := RealEigenvalues(M4.Eigenvalues);
  CmpR('sum eig(M4) = trace', ev[0]+ev[1]+ev[2]+ev[3], 16.0);
  ev := RealEigenvalues(M3.Eigenvalues);
  CmpR('sum eig(M3) = trace', ev[0]+ev[1]+ev[2], 6.0);

  // -- Rank --
  BeginCategory('Real rank');
  Track(IfThen(M3.Rank = 3, 0.0, 1.0));
  Track(IfThen(M4.Rank = 4, 0.0, 1.0));
  Track(IfThen(H4.Rank = 4, 0.0, 1.0));

  // -- Eigenvalues (5x5 symmetric - QR convergence stress) --
  BeginCategory('Real eigenvalues (5x5 symmetric)');
  S5.Init(TArrayOfDouble.Create(
    -0.14235884270194815, 0.9940954392257044, 0.058106254249750605, -0.05259808280494438, -0.443119050750149,
    0.9940954392257044, 0.7551804850779186, 0.755522308012678, 0.384873968241723, -1.053041066678274,
    0.058106254249750605, 0.755522308012678, 0.02350010848492948, -0.06194019222457631, -0.29265139847469945,
    -0.05259808280494438, 0.384873968241723, -0.06194019222457631, -0.5015653039496065, 0.20171369107721246,
    -0.443119050750149, -1.053041066678274, -0.29265139847469945, 0.20171369107721246, -0.3734862894349501));
  ev := RealEigenvalues(S5.Eigenvalues); SortAsc(ev);
  CmpR('eig(S5)[0]', ev[0], -1.311948101249088);
  CmpR('eig(S5)[1]', ev[1], -0.7562480655457912);
  CmpR('eig(S5)[2]', ev[2], -0.30347353323147597);
  CmpR('eig(S5)[3]', ev[3], -0.07801795708958013);
  CmpR('eig(S5)[4]', ev[4],  2.210957814592279);

  // -- Eigenvalues (degenerate spectrum 2,2,2,5,5) --
  BeginCategory('Real eigenvalues (degenerate)');
  D5.Init(TArrayOfDouble.Create(
    3.9681451856646284, -0.6470635423949582, -1.0683766715308218, 0.6550202348522072, -0.20412897125396062,
    -0.6470635423949582, 2.39008949997781, 0.07219196833991376, -0.2941787990675794, 0.712498847669345,
    -1.0683766715308218, 0.07219196833991376, 3.019026569770013, -0.23153568981902073, -0.904663136857451,
    0.6550202348522072, -0.2941787990675794, -0.23153568981902073, 2.253034895825433, -0.35479089535141245,
    -0.20412897125396062, 0.712498847669345, -0.904663136857451, -0.35479089535141245, 4.369703848762114));
  ev := RealEigenvalues(D5.Eigenvalues); SortAsc(ev);
  CmpR('eig(D5)[0]=2', ev[0], 2.0);
  CmpR('eig(D5)[1]=2', ev[1], 2.0);
  CmpR('eig(D5)[2]=2', ev[2], 2.0);
  CmpR('eig(D5)[3]=5', ev[3], 5.0);
  CmpR('eig(D5)[4]=5', ev[4], 5.0);
end;

procedure TestLargeRealInverse;
var
  M, Inv, Prod, Id: TRMatrix;
  i, j: integer;
begin
  // --- general real matrix, order 5 #1, cond=1.63 ---
  M.Init(TArrayOfDouble.Create(
    4.464600132057632, 0.6562114398235255, 0.05593869930914064, 0.8112058585830866, -0.664874906936775, 0.4715117142980525, 5.810568567370548, -0.2887185821356433,
    0.6295604388561127, 0.3315677508053454, -0.5141257122993563, 0.5687109426080146, 5.55476944777156, -0.012793148535181098, 0.17183074116486186, 0.2577061034992283,
    0.02534851804401428, -0.07966838246171459, 5.565743799499041, -0.3622834652402058, -0.335999068783676, 0.22021516675755848, 0.3336940833724975, -0.4748997191142621,
    4.868771577594321
  ));
  BeginCategory('Real det/trace/norm order 5 #1');
  CmpRRel('det',   M.Determinant, 3776.702013078322, 1e-9);
  CmpRRel('trace', M.Trace,       26.264453524293103,  1e-12);
  CmpRRel('norm',  M.Norm,        11.954056087316632, 1e-12);
  BeginCategory('Real inverse order 5 #1 (vs numpy, element-wise)');
  Inv := M.Inverse;
  CmpRRel('inv[0,0]', Inv[0,0], 0.23005976198307748, 1e-8);
  CmpRRel('inv[0,1]', Inv[0,1], -0.026463459628932918, 1e-8);
  CmpRRel('inv[0,2]', Inv[0,2], -0.005975720743830631, 1e-8);
  CmpRRel('inv[0,3]', Inv[0,3], -0.027876089344033263, 1e-8);
  CmpRRel('inv[0,4]', Inv[0,4], 0.03135558045239509, 1e-8);
  CmpRRel('inv[1,0]', Inv[1,0], -0.017345389307521247, 1e-8);
  CmpRRel('inv[1,1]', Inv[1,1], 0.1737743713929499, 1e-8);
  CmpRRel('inv[1,2]', Inv[1,2], 0.00989880980313908, 1e-8);
  CmpRRel('inv[1,3]', Inv[1,3], -0.018464282963598824, 1e-8);
  CmpRRel('inv[1,4]', Inv[1,4], -0.01592613548953108, 1e-8);
  CmpRRel('inv[2,0]', Inv[2,0], 0.022608408461884655, 1e-8);
  CmpRRel('inv[2,1]', Inv[2,1], -0.019983246922791634, 1e-8);
  CmpRRel('inv[2,2]', Inv[2,2], 0.17886335049490182, 1e-8);
  CmpRRel('inv[2,3]', Inv[2,3], -0.0007877400887874118, 1e-8);
  CmpRRel('inv[2,4]', Inv[2,4], -0.0019228759112186117, 1e-8);
  CmpRRel('inv[3,0]', Inv[3,0], -0.009325216510331885, 1e-8);
  CmpRRel('inv[3,1]', Inv[3,1], -0.00039600932119755326, 1e-8);
  CmpRRel('inv[3,2]', Inv[3,2], 0.0019503125611993633, 1e-8);
  CmpRRel('inv[3,3]', Inv[3,3], 0.18212300667800616, 1e-8);
  CmpRRel('inv[3,4]', Inv[3,4], 0.012236399021264265, 1e-8);
  CmpRRel('inv[4,0]', Inv[4,0], 0.014202093412701957, 1e-8);
  CmpRRel('inv[4,1]', Inv[4,1], -0.008355130810511365, 1e-8);
  CmpRRel('inv[4,2]', Inv[4,2], -0.012928752675776969, 1e-8);
  CmpRRel('inv[4,3]', Inv[4,3], 0.016729641700238024, 1e-8);
  CmpRRel('inv[4,4]', Inv[4,4], 0.20960016729575948, 1e-8);
  BeginCategory('Real inverse order 5 #1 (reconstruction A*inv=I)');
  Prod := M * Inv;
  Id   := M.Identity;
  for i := 0 to 4 do
    for j := 0 to 4 do
      CmpRAbs('M*inv-I', Prod[i,j], Id[i,j], 1e-9);
  // --- general real matrix, order 5 #2, cond=1.88 ---
  M.Init(TArrayOfDouble.Create(
    4.337154366099732, -0.6287805869624594, 0.917330432390359, 0.7127002817895094, 0.8843207336202106, 0.16954441073997573, 4.8874628592336995, -0.73470257216417,
    0.13577726480032348, -0.4090937187688306, -0.21090886786373808, 0.7394991587410129, 5.727251959888948, -0.9949531306398611, 0.8241307977342749, 0.5788244482089171,
    0.11209706807903785, -0.10565914307134316, 4.303839717458031, 0.14196753671343787, 0.23208035161269103, -0.38769686405650194, 0.048953675363139215, -0.6213240077760216,
    4.230472661223737
  ));
  BeginCategory('Real det/trace/norm order 5 #2');
  CmpRRel('det',   M.Determinant, 2181.1137015812815, 1e-9);
  CmpRRel('trace', M.Trace,       23.486181563904147,  1e-12);
  CmpRRel('norm',  M.Norm,        10.882678893357816, 1e-12);
  BeginCategory('Real inverse order 5 #2 (vs numpy, element-wise)');
  Inv := M.Inverse;
  CmpRRel('inv[0,0]', Inv[0,0], 0.23675473826061083, 1e-8);
  CmpRRel('inv[0,1]', Inv[0,1], 0.03387616692990263, 1e-8);
  CmpRRel('inv[0,2]', Inv[0,2], -0.03424211406500225, 1e-8);
  CmpRRel('inv[0,3]', Inv[0,3], -0.053639341841824444, 1e-8);
  CmpRRel('inv[0,4]', Inv[0,4], -0.03774367156902895, 1e-8);
  CmpRRel('inv[1,0]', Inv[1,0], -0.0078359575336906, 1e-8);
  CmpRRel('inv[1,1]', Inv[1,1], 0.2006872453526425, 1e-8);
  CmpRRel('inv[1,2]', Inv[1,2], 0.02692932174820268, 1e-8);
  CmpRRel('inv[1,3]', Inv[1,3], 0.0034558472137165134, 1e-8);
  CmpRRel('inv[1,4]', Inv[1,4], 0.01568276053471179, 1e-8);
  CmpRRel('inv[2,0]', Inv[2,0], 0.00700531999474801, 1e-8);
  CmpRRel('inv[2,1]', Inv[2,1], -0.028769422284713703, 1e-8);
  CmpRRel('inv[2,2]', Inv[2,2], 0.17074206186091773, 1e-8);
  CmpRRel('inv[2,3]', Inv[2,3], 0.033641492977694545, 1e-8);
  CmpRRel('inv[2,4]', Inv[2,4], -0.03863732123886156, 1e-8);
  CmpRRel('inv[3,0]', Inv[3,0], -0.03086082477710861, 1e-8);
  CmpRRel('inv[3,1]', Inv[3,1], -0.010992467429426278, 1e-8);
  CmpRRel('inv[3,2]', Inv[3,2], 0.007978694385871725, 1e-8);
  CmpRRel('inv[3,3]', Inv[3,3], 0.23904774203708612, 1e-8);
  CmpRRel('inv[3,4]', Inv[3,4], -0.004188323729771434, 1e-8);
  CmpRRel('inv[4,0]', Inv[4,0], -0.018319845739504383, 1e-8);
  CmpRRel('inv[4,1]', Inv[4,1], 0.015251796466786179, 1e-8);
  CmpRRel('inv[4,2]', Inv[4,2], 0.003542450116261187, 1e-8);
  CmpRRel('inv[4,3]', Inv[4,3], 0.0379786604748307, 1e-8);
  CmpRRel('inv[4,4]', Inv[4,4], 0.23971998773058623, 1e-8);
  BeginCategory('Real inverse order 5 #2 (reconstruction A*inv=I)');
  Prod := M * Inv;
  Id   := M.Identity;
  for i := 0 to 4 do
    for j := 0 to 4 do
      CmpRAbs('M*inv-I', Prod[i,j], Id[i,j], 1e-9);
  // --- general real matrix, order 6 #1, cond=1.69 ---
  M.Init(TArrayOfDouble.Create(
    6.696584559225658, 0.9764795662889532, 0.9173366500221565, -0.25408864396042397, -0.9237272317302081, 0.9251616408330574, 0.6483161770093875, 5.750713228492364,
    -0.6386514027296839, -0.3813109412283988, -0.2256207359992788, 0.7626090664741887, 0.11596190758196512, 0.5199661575524177, 5.756719270160521, 0.8510492250348294,
    0.9538742695287012, -0.907418381387584, 0.39859743161029315, 0.3571824226250191, 0.16319925082539233, 6.152524120501795, 0.8456291873972486, -0.04030843942427742,
    -0.42730692986596286, -0.9143601350785562, -0.6006966383089227, 0.9231343059130928, 6.9790458484595765, 0.6927937512902893, -0.5824819380131865, 0.7427489640272464,
    0.9858913283962738, 0.1019740401484539, -0.6151704390583215, 5.481450744565678
  ));
  BeginCategory('Real det/trace/norm order 6 #1');
  CmpRRel('det',   M.Determinant, 53634.391578387396, 1e-9);
  CmpRRel('trace', M.Trace,       36.817037771405595,  1e-12);
  CmpRRel('norm',  M.Norm,        15.542558587874089, 1e-12);
  BeginCategory('Real inverse order 6 #1 (vs numpy, element-wise)');
  Inv := M.Inverse;
  CmpRRel('inv[0,0]', Inv[0,0], 0.1497517205382073, 1e-8);
  CmpRRel('inv[0,1]', Inv[0,1], -0.017396114130827704, 1e-8);
  CmpRRel('inv[0,2]', Inv[0,2], -0.019138117138546913, 1e-8);
  CmpRRel('inv[0,3]', Inv[0,3], 0.005414846615854621, 1e-8);
  CmpRRel('inv[0,4]', Inv[0,4], 0.018719127215731476, 1e-8);
  CmpRRel('inv[0,5]', Inv[0,5], -0.028349177982069252, 1e-8);
  CmpRRel('inv[1,0]', Inv[1,0], -0.019551723625046848, 1e-8);
  CmpRRel('inv[1,1]', Inv[1,1], 0.17622231636261748, 1e-8);
  CmpRRel('inv[1,2]', Inv[1,2], 0.025036030440204034, 1e-8);
  CmpRRel('inv[1,3]', Inv[1,3], 0.00732828443380395, 1e-8);
  CmpRRel('inv[1,4]', Inv[1,4], -0.002670990243052744, 1e-8);
  CmpRRel('inv[1,5]', Inv[1,5], -0.016681029246874805, 1e-8);
  CmpRRel('inv[2,0]', Inv[2,0], 0.00211581321666465, 1e-8);
  CmpRRel('inv[2,1]', Inv[2,1], -0.020743711794403256, 1e-8);
  CmpRRel('inv[2,2]', Inv[2,2], 0.16442991035529772, 1e-8);
  CmpRRel('inv[2,3]', Inv[2,3], -0.021857123126151682, 1e-8);
  CmpRRel('inv[2,4]', Inv[2,4], -0.01741389086507625, 1e-8);
  CmpRRel('inv[2,5]', Inv[2,5], 0.03178935886490152, 1e-8);
  CmpRRel('inv[3,0]', Inv[3,0], -0.009341256134769814, 1e-8);
  CmpRRel('inv[3,1]', Inv[3,1], -0.01193776268292025, 1e-8);
  CmpRRel('inv[3,2]', Inv[3,2], -0.007605963977002201, 1e-8);
  CmpRRel('inv[3,3]', Inv[3,3], 0.16538455556640863, 1e-8);
  CmpRRel('inv[3,4]', Inv[3,4], -0.020116192654696847, 1e-8);
  CmpRRel('inv[3,5]', Inv[3,5], 0.005736983467279341, 1e-8);
  CmpRRel('inv[4,0]', Inv[4,0], 0.00613451986526241, 1e-8);
  CmpRRel('inv[4,1]', Inv[4,1], 0.023713536244022698, 1e-8);
  CmpRRel('inv[4,2]', Inv[4,2], 0.020499098676014002, 1e-8);
  CmpRRel('inv[4,3]', Inv[4,3], -0.022260786452509642, 1e-8);
  CmpRRel('inv[4,4]', Inv[4,4], 0.143068897050073, 1e-8);
  CmpRRel('inv[4,5]', Inv[4,5], -0.019187047664059154, 1e-8);
  CmpRRel('inv[5,0]', Inv[5,0], 0.019044241887073567, 1e-8);
  CmpRRel('inv[5,1]', Inv[5,1], -0.01911275246296401, 1e-8);
  CmpRRel('inv[5,2]', Inv[5,2], -0.03255836344967904, 1e-8);
  CmpRRel('inv[5,3]', Inv[5,3], -0.002061386077934647, 1e-8);
  CmpRRel('inv[5,4]', Inv[5,4], 0.021913672854285877, 1e-8);
  CmpRRel('inv[5,5]', Inv[5,5], 0.17370360444186608, 1e-8);
  BeginCategory('Real inverse order 6 #1 (reconstruction A*inv=I)');
  Prod := M * Inv;
  Id   := M.Identity;
  for i := 0 to 5 do
    for j := 0 to 5 do
      CmpRAbs('M*inv-I', Prod[i,j], Id[i,j], 1e-9);
  // --- general real matrix, order 6 #2, cond=1.81 ---
  M.Init(TArrayOfDouble.Create(
    6.3938768112421425, -0.1956233735108035, 0.4870507897637639, 0.648591401146299, 0.5348906261853137, -0.5767838074953968, 0.020203973647338058, 5.2619053338390325,
    -0.628862757799207, -0.8944805978443657, -0.748202160278761, 0.8708265516021823, 0.4078578056113058, -0.7654141481155421, 5.301351800072786, 0.4374342039612773,
    -0.593761458539815, -0.7040701933534241, 0.6362237219939635, 0.05125967025817446, -0.5649782082248371, 5.6717474303608615, -0.05813109913690728, 0.32613032879747217,
    0.41871302466786564, -0.8419703479182428, -0.12569610509793572, 0.5985387624615937, 6.091428380744137, 0.2580141184651945, 0.4016924323553632, -0.5313075604738464,
    -0.5335162253624728, 0.978705022121751, -0.17298731468830297, 6.223271162468301
  ));
  BeginCategory('Real det/trace/norm order 6 #2');
  CmpRRel('det',   M.Determinant, 36438.43446637963, 1e-9);
  CmpRRel('trace', M.Trace,       34.943580918727264,  1e-12);
  CmpRRel('norm',  M.Norm,        14.634723188877622, 1e-12);
  BeginCategory('Real inverse order 6 #2 (vs numpy, element-wise)');
  Inv := M.Inverse;
  CmpRRel('inv[0,0]', Inv[0,0], 0.15916986123386406, 1e-8);
  CmpRRel('inv[0,1]', Inv[0,1], 0.0029516001657609874, 1e-8);
  CmpRRel('inv[0,2]', Inv[0,2], -0.01505966063012312, 1e-8);
  CmpRRel('inv[0,3]', Inv[0,3], -0.017452486622848568, 1e-8);
  CmpRRel('inv[0,4]', Inv[0,4], -0.014846436419306469, 1e-8);
  CmpRRel('inv[0,5]', Inv[0,5], 0.014165473709808848, 1e-8);
  CmpRRel('inv[1,0]', Inv[1,0], -0.005300323194180388, 1e-8);
  CmpRRel('inv[1,1]', Inv[1,1], 0.19455757934849954, 1e-8);
  CmpRRel('inv[1,2]', Inv[1,2], 0.024750648864384706, 1e-8);
  CmpRRel('inv[1,3]', Inv[1,3], 0.031377421307121796, 1e-8);
  CmpRRel('inv[1,4]', Inv[1,4], 0.026289464793769736, 1e-8);
  CmpRRel('inv[1,5]', Inv[1,5], -0.027649933562688227, 1e-8);
  CmpRRel('inv[2,0]', Inv[2,0], -0.013778920929832638, 1e-8);
  CmpRRel('inv[2,1]', Inv[2,1], 0.0334526510484071, 1e-8);
  CmpRRel('inv[2,2]', Inv[2,2], 0.19459754050895597, 1e-8);
  CmpRRel('inv[2,3]', Inv[2,3], -0.013470224388519888, 1e-8);
  CmpRRel('inv[2,4]', Inv[2,4], 0.024605798736328236, 1e-8);
  CmpRRel('inv[2,5]', Inv[2,5], 0.01574345957642274, 1e-8);
  CmpRRel('inv[3,0]', Inv[3,0], -0.018748371082130302, 1e-8);
  CmpRRel('inv[3,1]', Inv[3,1], 0.00036635022326943553, 1e-8);
  CmpRRel('inv[3,2]', Inv[3,2], 0.019943872881144017, 1e-8);
  CmpRRel('inv[3,3]', Inv[3,3], 0.1780056992039738, 1e-8);
  CmpRRel('inv[3,4]', Inv[3,4], 0.005076439195860488, 1e-8);
  CmpRRel('inv[3,5]', Inv[3,5], -0.009071394551597849, 1e-8);
  CmpRRel('inv[4,0]', Inv[4,0], -0.009724843915382886, 1e-8);
  CmpRRel('inv[4,1]', Inv[4,1], 0.026497855499795605, 1e-8);
  CmpRRel('inv[4,2]', Inv[4,2], 0.005800812960528382, 1e-8);
  CmpRRel('inv[4,3]', Inv[4,3], -0.011145347227513345, 1e-8);
  CmpRRel('inv[4,4]', Inv[4,4], 0.16843883230980733, 1e-8);
  CmpRRel('inv[4,5]', Inv[4,5], -0.01035223253555369, 1e-8);
  CmpRRel('inv[5,0]', Inv[5,0], -0.009229528424403777, 1e-8);
  CmpRRel('inv[5,1]', Inv[5,1], 0.019966517662777364, 1e-8);
  CmpRRel('inv[5,2]', Inv[5,2], 0.01679258377690052, 1e-8);
  CmpRRel('inv[5,3]', Inv[5,3], -0.025653400213044728, 1e-8);
  CmpRRel('inv[5,4]', Inv[5,4], 0.009195892110055545, 1e-8);
  CmpRRel('inv[5,5]', Inv[5,5], 0.15990079771684, 1e-8);
  BeginCategory('Real inverse order 6 #2 (reconstruction A*inv=I)');
  Prod := M * Inv;
  Id   := M.Identity;
  for i := 0 to 5 do
    for j := 0 to 5 do
      CmpRAbs('M*inv-I', Prod[i,j], Id[i,j], 1e-9);
  // --- general real matrix, order 7 #1, cond=1.71 ---
  M.Init(TArrayOfDouble.Create(
    7.761796611320179, 0.7262296369747705, 0.1682250352414143, 0.62747740287845, -0.1192738838009264, 0.16540932592961943, -0.8318083252134258, -0.600425358283007,
    7.500453761824895, -0.3173329617152114, 0.6304382883509747, -0.6992450821326683, -0.5535728918641762, -0.27698968603457974, -0.18442383126925677, -0.9006732140736595,
    6.425129622199651, -0.7029621652321576, 0.016824203697030526, 0.7128751910877844, 0.6254878689899319, 0.35943323759673307, 0.12064518590891504, -0.6261672641023452,
    6.082945972745318, -0.4096334149354255, -0.3292972866207571, 0.967247747505791, -0.687972873802968, -0.7301908574999247, -0.05352265829764158, -0.44083005451889035,
    7.532323156334323, 0.448437741623388, -0.8696985879230672, -0.7295107012158475, 0.3352426053078088, -0.1135099250617786, 0.06846929812434199, -0.558919161625556,
    6.62354010945247, 0.4350231499035013, 0.9767270561100225, 0.8202591311707346, 0.2654206717085936, 0.20302470655037363, -0.3897930272001253, 0.1625218263012389,
    6.865176027991453
  ));
  BeginCategory('Real det/trace/norm order 7 #1');
  CmpRRel('det',   M.Determinant, 774830.8628913161, 1e-9);
  CmpRRel('trace', M.Trace,       48.791365261868286,  1e-12);
  CmpRRel('norm',  M.Norm,        18.845870547973146, 1e-12);
  BeginCategory('Real inverse order 7 #1 (vs numpy, element-wise)');
  Inv := M.Inverse;
  CmpRRel('inv[0,0]', Inv[0,0], 0.12558793884864022, 1e-8);
  CmpRRel('inv[0,1]', Inv[0,1], -0.014311456932419848, 1e-8);
  CmpRRel('inv[0,2]', Inv[0,2], -0.006024560595434665, 1e-8);
  CmpRRel('inv[0,3]', Inv[0,3], -0.012654533156339116, 1e-8);
  CmpRRel('inv[0,4]', Inv[0,4], 0.0005284567957436975, 1e-8);
  CmpRRel('inv[0,5]', Inv[0,5], -0.0047743942194625475, 1e-8);
  CmpRRel('inv[0,6]', Inv[0,6], 0.01734054524367794, 1e-8);
  CmpRRel('inv[1,0]', Inv[1,0], 0.011788122949678103, 1e-8);
  CmpRRel('inv[1,1]', Inv[1,1], 0.13282406911520092, 1e-8);
  CmpRRel('inv[1,2]', Inv[1,2], 0.004764878231967872, 1e-8);
  CmpRRel('inv[1,3]', Inv[1,3], -0.01390832268345498, 1e-8);
  CmpRRel('inv[1,4]', Inv[1,4], 0.012867456329968788, 1e-8);
  CmpRRel('inv[1,5]', Inv[1,5], 0.008500372127435669, 1e-8);
  CmpRRel('inv[1,6]', Inv[1,6], 0.009404234639425175, 1e-8);
  CmpRRel('inv[2,0]', Inv[2,0], 0.005110644571348844, 1e-8);
  CmpRRel('inv[2,1]', Inv[2,1], 0.020544278041120315, 1e-8);
  CmpRRel('inv[2,2]', Inv[2,2], 0.15845558151103564, 1e-8);
  CmpRRel('inv[2,3]', Inv[2,3], 0.016345773328635113, 1e-8);
  CmpRRel('inv[2,4]', Inv[2,4], 0.0007187263629022805, 1e-8);
  CmpRRel('inv[2,5]', Inv[2,5], -0.014350130266449976, 1e-8);
  CmpRRel('inv[2,6]', Inv[2,6], -0.014291419774699946, 1e-8);
  CmpRRel('inv[3,0]', Inv[3,0], -0.0026066708458189628, 1e-8);
  CmpRRel('inv[3,1]', Inv[3,1], 0.002929601139855311, 1e-8);
  CmpRRel('inv[3,2]', Inv[3,2], 0.01777986030352745, 1e-8);
  CmpRRel('inv[3,3]', Inv[3,3], 0.16778415595664584, 1e-8);
  CmpRRel('inv[3,4]', Inv[3,4], 0.008534277556401461, 1e-8);
  CmpRRel('inv[3,5]', Inv[3,5], 0.006768764622207625, 1e-8);
  CmpRRel('inv[3,6]', Inv[3,6], -0.024804758761023372, 1e-8);
  CmpRRel('inv[4,0]', Inv[4,0], 0.009359886317154441, 1e-8);
  CmpRRel('inv[4,1]', Inv[4,1], 0.010640846260977851, 1e-8);
  CmpRRel('inv[4,2]', Inv[4,2], 0.0012176616793621931, 1e-8);
  CmpRRel('inv[4,3]', Inv[4,3], 0.0073216618350250475, 1e-8);
  CmpRRel('inv[4,4]', Inv[4,4], 0.13458214647111086, 1e-8);
  CmpRRel('inv[4,5]', Inv[4,5], -0.008665304925964798, 1e-8);
  CmpRRel('inv[4,6]', Inv[4,6], 0.018019210721574058, 1e-8);
  CmpRRel('inv[5,0]', Inv[5,0], 0.015402838335158044, 1e-8);
  CmpRRel('inv[5,1]', Inv[5,1], -0.006162098913713412, 1e-8);
  CmpRRel('inv[5,2]', Inv[5,2], 0.0021465417554409148, 1e-8);
  CmpRRel('inv[5,3]', Inv[5,3], -0.0014157816752753997, 1e-8);
  CmpRRel('inv[5,4]', Inv[5,4], 0.010326073835715046, 1e-8);
  CmpRRel('inv[5,5]', Inv[5,5], 0.14923662890631595, 1e-8);
  CmpRRel('inv[5,6]', Inv[5,6], -0.006526951275197753, 1e-8);
  CmpRRel('inv[6,0]', Inv[6,0], -0.019229890664304534, 1e-8);
  CmpRRel('inv[6,1]', Inv[6,1], -0.013964714405819114, 1e-8);
  CmpRRel('inv[6,2]', Inv[6,2], -0.0063458598908185705, 1e-8);
  CmpRRel('inv[6,3]', Inv[6,3], -0.0016824555357878833, 1e-8);
  CmpRRel('inv[6,4]', Inv[6,4], 0.005504116590337264, 1e-8);
  CmpRRel('inv[6,5]', Inv[6,5], -0.0040066733743381565, 1e-8);
  CmpRRel('inv[6,6]', Inv[6,6], 0.1445356766896434, 1e-8);
  BeginCategory('Real inverse order 7 #1 (reconstruction A*inv=I)');
  Prod := M * Inv;
  Id   := M.Identity;
  for i := 0 to 6 do
    for j := 0 to 6 do
      CmpRAbs('M*inv-I', Prod[i,j], Id[i,j], 1e-9);
  // --- general real matrix, order 7 #2, cond=1.49 ---
  M.Init(TArrayOfDouble.Create(
    7.973042603552368, 0.31474999730408015, -0.013139253082802282, -0.3868546286293073, -0.42944755064894014, -0.08388653307643157, 0.17219901381645397, 0.8568714551682437,
    6.821382808643177, 0.709846860710428, -0.5667770214823447, 0.14243752785674801, -0.3290176490385808, -0.3207761578467474, 0.07263391084515036, -0.3539358679417801,
    7.390428477042728, 0.9016149451909361, -0.758500229690255, -0.06078425625812667, -0.3208555347624493, -0.02708838605907249, -0.13686331614302905, -0.5934643752423716,
    7.648524469045494, 0.07412892501299306, -0.5374272996868992, 0.7047254671735022, -0.5337150554792236, -0.7708064603167872, -0.27838679526691057, -0.4096690821060853,
    7.408359118963802, 0.9857643709312314, -0.9043701409884866, 0.13912779639411776, 0.4746936366491148, -0.7403224965578745, -0.21981906317971367, -0.1852421093556753,
    7.906811270814808, -0.07171208363706238, -0.9769218907985864, 0.6937979883247165, -0.4470125566952363, -0.40640865147634053, -0.7218196744092427, -0.7079541461278169,
    7.156789617907356
  ));
  BeginCategory('Real det/trace/norm order 7 #2');
  CmpRRel('det',   M.Determinant, 1291844.8291557697, 1e-9);
  CmpRRel('trace', M.Trace,       52.30533836596973,  1e-12);
  CmpRRel('norm',  M.Norm,        20.08746851037215, 1e-12);
  BeginCategory('Real inverse order 7 #2 (vs numpy, element-wise)');
  Inv := M.Inverse;
  CmpRRel('inv[0,0]', Inv[0,0], 0.12604353638079635, 1e-8);
  CmpRRel('inv[0,1]', Inv[0,1], -0.0045500997037116785, 1e-8);
  CmpRRel('inv[0,2]', Inv[0,2], 0.0012930920468615006, 1e-8);
  CmpRRel('inv[0,3]', Inv[0,3], 0.0061306295829962, 1e-8);
  CmpRRel('inv[0,4]', Inv[0,4], 0.007196015524647737, 1e-8);
  CmpRRel('inv[0,5]', Inv[0,5], 0.00042053384876843315, 1e-8);
  CmpRRel('inv[0,6]', Inv[0,6], -0.0028688327285393824, 1e-8);
  CmpRRel('inv[1,0]', Inv[1,0], -0.015325422254032906, 1e-8);
  CmpRRel('inv[1,1]', Inv[1,1], 0.14540485514907878, 1e-8);
  CmpRRel('inv[1,2]', Inv[1,2], -0.01218011861691272, 1e-8);
  CmpRRel('inv[1,3]', Inv[1,3], 0.011667418154089605, 1e-8);
  CmpRRel('inv[1,4]', Inv[1,4], -0.00439986260354769, 1e-8);
  CmpRRel('inv[1,5]', Inv[1,5], 0.0075577151090794, 1e-8);
  CmpRRel('inv[1,6]', Inv[1,6], 0.004710760706776999, 1e-8);
  CmpRRel('inv[2,0]', Inv[2,0], 9.889681648059321e-05, 1e-8);
  CmpRRel('inv[2,1]', Inv[2,1], 0.007420807839708387, 1e-8);
  CmpRRel('inv[2,2]', Inv[2,2], 0.13454700668851213, 1e-8);
  CmpRRel('inv[2,3]', Inv[2,3], -0.01402521040697124, 1e-8);
  CmpRRel('inv[2,4]', Inv[2,4], 0.014699206191221285, 1e-8);
  CmpRRel('inv[2,5]', Inv[2,5], -0.0005825952640904979, 1e-8);
  CmpRRel('inv[2,6]', Inv[2,6], 0.009594973549887508, 1e-8);
  CmpRRel('inv[3,0]', Inv[3,0], -0.0017782794221301561, 1e-8);
  CmpRRel('inv[3,1]', Inv[3,1], 0.003710375667997249, 1e-8);
  CmpRRel('inv[3,2]', Inv[3,2], 0.01004035725512876, 1e-8);
  CmpRRel('inv[3,3]', Inv[3,3], 0.1292552707711942, 1e-8);
  CmpRRel('inv[3,4]', Inv[3,4], -0.0014230941104862662, 1e-8);
  CmpRRel('inv[3,5]', Inv[3,5], 0.008086214167545243, 1e-8);
  CmpRRel('inv[3,6]', Inv[3,6], -0.012167282828267597, 1e-8);
  CmpRRel('inv[4,0]', Inv[4,0], 0.009895679427131208, 1e-8);
  CmpRRel('inv[4,1]', Inv[4,1], 0.014674294145532113, 1e-8);
  CmpRRel('inv[4,2]', Inv[4,2], 0.004079936748736771, 1e-8);
  CmpRRel('inv[4,3]', Inv[4,3], 0.008927913287626559, 1e-8);
  CmpRRel('inv[4,4]', Inv[4,4], 0.1368920627342206, 1e-8);
  CmpRRel('inv[4,5]', Inv[4,5], -0.01420156294818971, 1e-8);
  CmpRRel('inv[4,6]', Inv[4,6], 0.016879516757694474, 1e-8);
  CmpRRel('inv[5,0]', Inv[5,0], -0.0009292386909532889, 1e-8);
  CmpRRel('inv[5,1]', Inv[5,1], -0.007628490513170248, 1e-8);
  CmpRRel('inv[5,2]', Inv[5,2], 0.013790786530818779, 1e-8);
  CmpRRel('inv[5,3]', Inv[5,3], 0.0017467770618131895, 1e-8);
  CmpRRel('inv[5,4]', Inv[5,4], 0.0048313250701728025, 1e-8);
  CmpRRel('inv[5,5]', Inv[5,5], 0.12594735753642092, 1e-8);
  CmpRRel('inv[5,6]', Inv[5,6], 0.0019992318276332677, 1e-8);
  CmpRRel('inv[6,0]', Inv[6,0], 0.01950231628437745, 1e-8);
  CmpRRel('inv[6,1]', Inv[6,1], -0.013317422657049825, 1e-8);
  CmpRRel('inv[6,2]', Inv[6,2], 0.012106923491354941, 1e-8);
  CmpRRel('inv[6,3]', Inv[6,3], 0.0072429559091516625, 1e-8);
  CmpRRel('inv[6,4]', Inv[6,4], 0.016530690432632934, 1e-8);
  CmpRRel('inv[6,5]', Inv[6,5], 0.010773988298606376, 1e-8);
  CmpRRel('inv[6,6]', Inv[6,6], 0.1406877424604634, 1e-8);
  BeginCategory('Real inverse order 7 #2 (reconstruction A*inv=I)');
  Prod := M * Inv;
  Id   := M.Identity;
  for i := 0 to 6 do
    for j := 0 to 6 do
      CmpRAbs('M*inv-I', Prod[i,j], Id[i,j], 1e-9);
  // --- general real matrix, order 8 #1, cond=1.57 ---
  M.Init(TArrayOfDouble.Create(
    8.679366942343949, 0.27125239148089686, 0.08364802528462456, -0.49508273295225513, -0.36640601295898234, -0.49891537143664677, -0.36205756145592227, -0.23350963973559158,
    0.32876879798033465, 7.271618350255698, -0.6764952339453238, -0.8783722194737282, 0.6477055860238636, 0.17547842607649367, -0.9076836525543805, 0.8828335002581038,
    0.5087292481362922, -0.9926669303581988, 7.818784494920059, 0.29071564603595434, 0.4672484735427591, 0.5839632753220518, -0.5710301903742667, -0.20621378430110426,
    -0.5910953524649136, 0.06219704403498283, -0.12142689431968967, 7.237189400868017, 0.10069562986847447, 0.260897652905687, 0.7925989727070082, -0.02367369168933009,
    0.5581274790978581, 0.6631535001389695, 0.36786996163888164, -0.9088507071481366, 8.638869175585606, -0.06017142964281552, -0.5163224439990715, -0.6384663411431122,
    -0.2272341346210882, 0.19961894161951532, 0.28655061412932414, 0.5125282441956744, 0.38435440914202723, 7.985845993289648, 0.8503181341227628, 0.16108735600905222,
    0.6589484054940278, 0.7693624018379008, -0.8457279856427256, 0.38803925457657673, 0.14908034036739526, -0.3189070883701397, 8.39709082865754, -0.6787107750997863,
    -0.3168299962220962, -0.8274467286918143, -0.4307974551290292, 0.4608057309445821, 0.5420184554722265, 0.43567247955604915, 0.06264987271668399, 7.473693094104548
  ));
  BeginCategory('Real det/trace/norm order 8 #1');
  CmpRRel('det',   M.Determinant, 15271244.818469532, 1e-9);
  CmpRRel('trace', M.Trace,       63.50245828002507,  1e-12);
  CmpRRel('norm',  M.Norm,        22.846807848091704, 1e-12);
  BeginCategory('Real inverse order 8 #1 (vs numpy, element-wise)');
  Inv := M.Inverse;
  CmpRRel('inv[0,0]', Inv[0,0], 0.1157813756165526, 1e-8);
  CmpRRel('inv[0,1]', Inv[0,1], -0.00499392523446439, 1e-8);
  CmpRRel('inv[0,2]', Inv[0,2], -0.0014317071690996643, 1e-8);
  CmpRRel('inv[0,3]', Inv[0,3], 0.006972787844662169, 1e-8);
  CmpRRel('inv[0,4]', Inv[0,4], 0.004612286961192433, 1e-8);
  CmpRRel('inv[0,5]', Inv[0,5], 0.0071258715123613805, 1e-8);
  CmpRRel('inv[0,6]', Inv[0,6], 0.0032235818425940927, 1e-8);
  CmpRRel('inv[0,7]', Inv[0,7], 0.00472316526048392, 1e-8);
  CmpRRel('inv[1,0]', Inv[1,0], -0.006012502204795648, 1e-8);
  CmpRRel('inv[1,1]', Inv[1,1], 0.13727210000779178, 1e-8);
  CmpRRel('inv[1,2]', Inv[1,2], 0.01344712943183589, 1e-8);
  CmpRRel('inv[1,3]', Inv[1,3], 0.01487795405589771, 1e-8);
  CmpRRel('inv[1,4]', Inv[1,4], -0.010555131716523818, 1e-8);
  CmpRRel('inv[1,5]', Inv[1,5], -0.0035370858280846474, 1e-8);
  CmpRRel('inv[1,6]', Inv[1,6], 0.013914466834893063, 1e-8);
  CmpRRel('inv[1,7]', Inv[1,7], -0.015546879261118206, 1e-8);
  CmpRRel('inv[2,0]', Inv[2,0], -0.009255020183382243, 1e-8);
  CmpRRel('inv[2,1]', Inv[2,1], 0.01840223705223403, 1e-8);
  CmpRRel('inv[2,2]', Inv[2,2], 0.1316280164568888, 1e-8);
  CmpRRel('inv[2,3]', Inv[2,3], -0.004780911677744455, 1e-8);
  CmpRRel('inv[2,4]', Inv[2,4], -0.008687205051016916, 1e-8);
  CmpRRel('inv[2,5]', Inv[2,5], -0.010150151362504588, 1e-8);
  CmpRRel('inv[2,6]', Inv[2,6], 0.011473754629059788, 1e-8);
  CmpRRel('inv[2,7]', Inv[2,7], 0.0016724015221133669, 1e-8);
  CmpRRel('inv[3,0]', Inv[3,0], 0.010318924115456178, 1e-8);
  CmpRRel('inv[3,1]', Inv[3,1], 6.831346962315176e-06, 1e-8);
  CmpRRel('inv[3,2]', Inv[3,2], 0.0009178862995913018, 1e-8);
  CmpRRel('inv[3,3]', Inv[3,3], 0.13973674901060787, 1e-8);
  CmpRRel('inv[3,4]', Inv[3,4], -0.0008119148224826341, 1e-8);
  CmpRRel('inv[3,5]', Inv[3,5], -0.004467900435683394, 1e-8);
  CmpRRel('inv[3,6]', Inv[3,6], -0.012276894181391496, 1e-8);
  CmpRRel('inv[3,7]', Inv[3,7], -0.0002984087684188951, 1e-8);
  CmpRRel('inv[4,0]', Inv[4,0], -0.005825581767498803, 1e-8);
  CmpRRel('inv[4,1]', Inv[4,1], -0.010290892338341951, 1e-8);
  CmpRRel('inv[4,2]', Inv[4,2], -0.005024442697404023, 1e-8);
  CmpRRel('inv[4,3]', Inv[4,3], 0.012121715875582221, 1e-8);
  CmpRRel('inv[4,4]', Inv[4,4], 0.11562602070501653, 1e-8);
  CmpRRel('inv[4,5]', Inv[4,5], 0.0002604642968010181, 1e-8);
  CmpRRel('inv[4,6]', Inv[4,6], 0.004150415539298269, 1e-8);
  CmpRRel('inv[4,7]', Inv[4,7], 0.011182419052631322, 1e-8);
  CmpRRel('inv[5,0]', Inv[5,0], 0.004329702259985901, 1e-8);
  CmpRRel('inv[5,1]', Inv[5,1], -0.0031361810159082327, 1e-8);
  CmpRRel('inv[5,2]', Inv[5,2], -0.006464366530225684, 1e-8);
  CmpRRel('inv[5,3]', Inv[5,3], -0.00834582411798762, 1e-8);
  CmpRRel('inv[5,4]', Inv[5,4], -0.00427117352983086, 1e-8);
  CmpRRel('inv[5,5]', Inv[5,5], 0.12597822263263922, 1e-8);
  CmpRRel('inv[5,6]', Inv[5,6], -0.012794368245490415, 1e-8);
  CmpRRel('inv[5,7]', Inv[5,7], -0.003941162638909367, 1e-8);
  CmpRRel('inv[6,0]', Inv[6,0], -0.009407508515980921, 1e-8);
  CmpRRel('inv[6,1]', Inv[6,1], -0.008890503708750731, 1e-8);
  CmpRRel('inv[6,2]', Inv[6,2], 0.012714130198627218, 1e-8);
  CmpRRel('inv[6,3]', Inv[6,3], -0.00996802168040223, 1e-8);
  CmpRRel('inv[6,4]', Inv[6,4], -0.003217852384849295, 1e-8);
  CmpRRel('inv[6,5]', Inv[6,5], 0.003099411529414516, 1e-8);
  CmpRRel('inv[6,6]', Inv[6,6], 0.11892992935005356, 1e-8);
  CmpRRel('inv[6,7]', Inv[6,7], 0.011534216967914521, 1e-8);
  CmpRRel('inv[7,0]', Inv[7,0], 0.0033218582323119752, 1e-8);
  CmpRRel('inv[7,1]', Inv[7,1], 0.017050312663005055, 1e-8);
  CmpRRel('inv[7,2]', Inv[7,2], 0.009593432651274103, 1e-8);
  CmpRRel('inv[7,3]', Inv[7,3], -0.007257569778235406, 1e-8);
  CmpRRel('inv[7,4]', Inv[7,4], -0.00953341201294864, 1e-8);
  CmpRRel('inv[7,5]', Inv[7,5], -0.007787780898751319, 1e-8);
  CmpRRel('inv[7,6]', Inv[7,6], 0.0025433934898517543, 1e-8);
  CmpRRel('inv[7,7]', Inv[7,7], 0.13171848908051614, 1e-8);
  BeginCategory('Real inverse order 8 #1 (reconstruction A*inv=I)');
  Prod := M * Inv;
  Id   := M.Identity;
  for i := 0 to 7 do
    for j := 0 to 7 do
      CmpRAbs('M*inv-I', Prod[i,j], Id[i,j], 1e-9);
  // --- general real matrix, order 8 #2, cond=1.71 ---
  M.Init(TArrayOfDouble.Create(
    8.62520070811119, 0.9670855956168274, -0.14928643362077554, 0.4699234922157809, 0.8803754863392808, 0.9528370467488911, -0.6727788315143348, 0.37469264501442656,
    -0.15011099214581014, 7.369902282572006, 0.5791778267766876, 0.5632273996999897, 0.05660335590002785, -0.31878738517399197, -0.9596616502142654, -0.24301481464422414,
    -0.8136129096903901, -0.2931355700221534, 8.212837544743046, -0.7795415227822644, 0.039081031444150094, -0.013152306519579993, -0.9270280280862195, -0.5842440213809366,
    -0.9720999358779927, 0.4711131149367582, -0.10997708350546187, 7.581320762522733, -0.21615935974743228, -0.8614638071756302, 0.9663756880049845, 0.19381858768869265,
    0.35901776006966957, 0.8389575654863937, 0.3997006590707253, 0.262335132076676, 7.79771439409829, -0.4311862596452476, -0.7222035761711005, -0.7251474634899309,
    0.3066905059575764, 0.6943419343665296, -0.7897381862330195, 0.9819352073068206, 0.05466384543164038, 7.139707577289147, 0.7324633617322078, -0.5974217579491641,
    0.5799057577081417, -0.5109195385817744, -0.6762381291988606, 0.7716771363411128, -0.0698893665283471, -0.487784522407106, 7.21771295567365, -0.8460913013548179,
    0.8243329755583999, -0.38012842314624073, -0.15791617741112196, -0.5322912158539692, 0.40818404217842463, 0.4600510237798314, -0.36277040917715153, 7.057882259756383
  ));
  BeginCategory('Real det/trace/norm order 8 #2');
  CmpRRel('det',   M.Determinant, 11011681.573904397, 1e-9);
  CmpRRel('trace', M.Trace,       61.00227848476645,  1e-12);
  CmpRRel('norm',  M.Norm,        22.082647421096972, 1e-12);
  BeginCategory('Real inverse order 8 #2 (vs numpy, element-wise)');
  Inv := M.Inverse;
  CmpRRel('inv[0,0]', Inv[0,0], 0.11654456219829523, 1e-8);
  CmpRRel('inv[0,1]', Inv[0,1], -0.011569653360577662, 1e-8);
  CmpRRel('inv[0,2]', Inv[0,2], 0.0026224946222935235, 1e-8);
  CmpRRel('inv[0,3]', Inv[0,3], -0.005144951712391097, 1e-8);
  CmpRRel('inv[0,4]', Inv[0,4], -0.012622295103416021, 1e-8);
  CmpRRel('inv[0,5]', Inv[0,5], -0.01624770365041336, 1e-8);
  CmpRRel('inv[0,6]', Inv[0,6], 0.010351699242449802, 1e-8);
  CmpRRel('inv[0,7]', Inv[0,7], -0.007658373382097711, 1e-8);
  CmpRRel('inv[1,0]', Inv[1,0], -0.0018546739728661082, 1e-8);
  CmpRRel('inv[1,1]', Inv[1,1], 0.13773980049665033, 1e-8);
  CmpRRel('inv[1,2]', Inv[1,2], -0.007657101554870549, 1e-8);
  CmpRRel('inv[1,3]', Inv[1,3], -0.012965328768254469, 1e-8);
  CmpRRel('inv[1,4]', Inv[1,4], -0.0013566550560717822, 1e-8);
  CmpRRel('inv[1,5]', Inv[1,5], 0.005546072780814126, 1e-8);
  CmpRRel('inv[1,6]', Inv[1,6], 0.018552518636380702, 1e-8);
  CmpRRel('inv[1,7]', Inv[1,7], 0.007117396274826339, 1e-8);
  CmpRRel('inv[2,0]', Inv[2,0], 0.010831453931761768, 1e-8);
  CmpRRel('inv[2,1]', Inv[2,1], 0.004694870918940891, 1e-8);
  CmpRRel('inv[2,2]', Inv[2,2], 0.12348796286895121, 1e-8);
  CmpRRel('inv[2,3]', Inv[2,3], 0.010804093291468617, 1e-8);
  CmpRRel('inv[2,4]', Inv[2,4], -0.002025871452061409, 1e-8);
  CmpRRel('inv[2,5]', Inv[2,5], 0.0005614119511660678, 1e-8);
  CmpRRel('inv[2,6]', Inv[2,6], 0.016356722293081636, 1e-8);
  CmpRRel('inv[2,7]', Inv[2,7], 0.011312338271051964, 1e-8);
  CmpRRel('inv[3,0]', Inv[3,0], 0.01618043346440914, 1e-8);
  CmpRRel('inv[3,1]', Inv[3,1], -0.013468156518998783, 1e-8);
  CmpRRel('inv[3,2]', Inv[3,2], 0.002430078464915045, 1e-8);
  CmpRRel('inv[3,3]', Inv[3,3], 0.13200901397976872, 1e-8);
  CmpRRel('inv[3,4]', Inv[3,4], 0.0019635898449245773, 1e-8);
  CmpRRel('inv[3,5]', Inv[3,5], 0.01236478671711585, 1e-8);
  CmpRRel('inv[3,6]', Inv[3,6], -0.018993593656560597, 1e-8);
  CmpRRel('inv[3,7]', Inv[3,7], -0.00577526244714505, 1e-8);
  CmpRRel('inv[4,0]', Inv[4,0], -0.008807196929504461, 1e-8);
  CmpRRel('inv[4,1]', Inv[4,1], -0.012560167532558422, 1e-8);
  CmpRRel('inv[4,2]', Inv[4,2], -0.0037074574847127248, 1e-8);
  CmpRRel('inv[4,3]', Inv[4,3], -0.004346266704488804, 1e-8);
  CmpRRel('inv[4,4]', Inv[4,4], 0.12848449146649876, 1e-8);
  CmpRRel('inv[4,5]', Inv[4,5], 0.007593954793951726, 1e-8);
  CmpRRel('inv[4,6]', Inv[4,6], 0.010451420638249399, 1e-8);
  CmpRRel('inv[4,7]', Inv[4,7], 0.014944123696992207, 1e-8);
  CmpRRel('inv[5,0]', Inv[5,0], -0.005550693373626373, 1e-8);
  CmpRRel('inv[5,1]', Inv[5,1], -0.010917338212723195, 1e-8);
  CmpRRel('inv[5,2]', Inv[5,2], 0.012973414842295333, 1e-8);
  CmpRRel('inv[5,3]', Inv[5,3], -0.013199191715234163, 1e-8);
  CmpRRel('inv[5,4]', Inv[5,4], -0.001389424057176083, 1e-8);
  CmpRRel('inv[5,5]', Inv[5,5], 0.13710838100904793, 1e-8);
  CmpRRel('inv[5,6]', Inv[5,6], -0.012016564347482, 1e-8);
  CmpRRel('inv[5,7]', Inv[5,7], 0.011377562981966278, 1e-8);
  CmpRRel('inv[6,0]', Inv[6,0], -0.012077097311952207, 1e-8);
  CmpRRel('inv[6,1]', Inv[6,1], 0.012867488641082657, 1e-8);
  CmpRRel('inv[6,2]', Inv[6,2], 0.011655451424862004, 1e-8);
  CmpRRel('inv[6,3]', Inv[6,3], -0.013305781229429327, 1e-8);
  CmpRRel('inv[6,4]', Inv[6,4], 0.0009903971957593446, 1e-8);
  CmpRRel('inv[6,5]', Inv[6,5], 0.009092080659570515, 1e-8);
  CmpRRel('inv[6,6]', Inv[6,6], 0.1426125127725485, 1e-8);
  CmpRRel('inv[6,7]', Inv[6,7], 0.020382023600376106, 1e-8);
  CmpRRel('inv[7,0]', Inv[7,0], -0.011998787545173018, 1e-8);
  CmpRRel('inv[7,1]', Inv[7,1], 0.009958484530705096, 1e-8);
  CmpRRel('inv[7,2]', Inv[7,2], 0.002195405116212513, 1e-8);
  CmpRRel('inv[7,3]', Inv[7,3], 0.010528013216943861, 1e-8);
  CmpRRel('inv[7,4]', Inv[7,4], -0.005785343271709339, 1e-8);
  CmpRRel('inv[7,5]', Inv[7,5], -0.005767478361776042, 1e-8);
  CmpRRel('inv[7,6]', Inv[7,6], 0.006232703836034443, 1e-8);
  CmpRRel('inv[7,7]', Inv[7,7], 0.14222263844465532, 1e-8);
  BeginCategory('Real inverse order 8 #2 (reconstruction A*inv=I)');
  Prod := M * Inv;
  Id   := M.Identity;
  for i := 0 to 7 do
    for j := 0 to 7 do
      CmpRAbs('M*inv-I', Prod[i,j], Id[i,j], 1e-9);
end;

procedure TestLargeRealEigen;
var
  M: TRMatrix;
  ev: TArrayOfDouble;
  s: double;
  i: integer;
begin
  // --- symmetric, order 5 #1 (well-sep) ---
  M.Init(TArrayOfDouble.Create(
    1.877038618100526, 0.37675191946407327, 2.2546487118191565, -0.7746835299354351, -0.030433597245280775, 0.37675191946407327, 1.3423252971070254, -1.9613643482167642,
    0.8960361461072767, 0.318857598837507, 2.2546487118191565, -1.9613643482167642, 0.9917374187508881, -1.014298552761613, -2.0494425368630704, -0.7746835299354351,
    0.8960361461072767, -1.014298552761613, 0.2460588289725014, -0.4187620819579271, -0.030433597245280775, 0.318857598837507, -2.0494425368630704, -0.4187620819579271,
    -0.957160162930941
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 5 #1 (well-sep)');
  CmpRRel('eig[0]', ev[0], -2.9999999999999987, 1e-6);
  CmpRRel('eig[1]', ev[1], -1.0000000000000002, 1e-6);
  CmpRRel('eig[2]', ev[2], 0.5, 1e-6);
  CmpRRel('eig[3]', ev[3], 1.9999999999999993, 1e-6);
  CmpRRel('eig[4]', ev[4], 5.0, 1e-6);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 3.5, 1e-9);
  // --- symmetric, order 5 #2 (well-sep) ---
  M.Init(TArrayOfDouble.Create(
    3.9932502297285004, 0.2774892985808819, 0.9195588924553463, 0.4854068576102204, -0.07896346913254619, 0.2774892985808819, 2.6142794805711964, -0.8256175367371235,
    0.49715476094451855, -0.971990534170303, 0.9195588924553463, -0.8256175367371235, 2.8271658623359515, 0.8987240463996049, 0.4616652277387977, 0.4854068576102204,
    0.49715476094451855, 0.8987240463996049, 2.9743108867097865, 0.5239957289859325, -0.07896346913254619, -0.971990534170303, 0.4616652277387977, 0.5239957289859325,
    2.5909935406545666
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 5 #2 (well-sep)');
  CmpRRel('eig[0]', ev[0], 0.9999999999999997, 1e-6);
  CmpRRel('eig[1]', ev[1], 2.0000000000000004, 1e-6);
  CmpRRel('eig[2]', ev[2], 3.0, 1e-6);
  CmpRRel('eig[3]', ev[3], 3.9999999999999987, 1e-6);
  CmpRRel('eig[4]', ev[4], 5.000000000000001, 1e-6);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 15.0, 1e-9);
  // --- symmetric, order 6 #1 (well-sep) ---
  M.Init(TArrayOfDouble.Create(
    -0.6121084908600687, 2.245343023674996, 0.16674229500224264, -0.49847519974089755, 1.5221706349740396, 0.20748750728111692, 2.245343023674996, -1.2032325169660814,
    1.5563671977822997, 3.17414482661633, -0.16417971387666466, 0.3563533913707971, 0.16674229500224264, 1.5563671977822997, 0.8012098925577938, -1.295853738177847,
    0.4276105071339328, -1.858701979390446, -0.49847519974089755, 3.17414482661633, -1.295853738177847, 2.3691910552547886, -0.5849273230609542, 2.8333598073865778,
    1.5221706349740396, -0.16417971387666466, 0.4276105071339328, -0.5849273230609542, -1.1444600586666092, -0.0711255735728339, 0.20748750728111692, 0.3563533913707971,
    -1.858701979390446, 2.8333598073865778, -0.0711255735728339, 3.289400118680177
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 6 #1 (well-sep)');
  CmpRRel('eig[0]', ev[0], -5.000000000000001, 1e-6);
  CmpRRel('eig[1]', ev[1], -2.0, 1e-6);
  CmpRRel('eig[2]', ev[2], -0.5000000000000007, 1e-6);
  CmpRRel('eig[3]', ev[3], 0.9999999999999996, 1e-6);
  CmpRRel('eig[4]', ev[4], 3.000000000000002, 1e-6);
  CmpRRel('eig[5]', ev[5], 6.999999999999998, 1e-6);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 3.5, 1e-9);
  // --- symmetric, order 6 #2 (well-sep) ---
  M.Init(TArrayOfDouble.Create(
    2.480402734497463, 1.441374863657384, 0.3670830522428436, 0.7575301121341951, 0.35746768233559023, -0.11114766738372994, 1.441374863657384, 3.7732825343897685,
    -0.5157016692239338, 0.6810159404958745, 0.3753263455229465, 0.6011392517099333, 0.3670830522428436, -0.5157016692239338, 2.214517669948381, -0.8059040074582409,
    -1.7958168118429223, -0.09141629028725604, 0.7575301121341951, 0.6810159404958745, -0.8059040074582409, 4.470035011193196, 1.6015415642401545, -0.5555025428202363,
    0.35746768233559023, 0.3753263455229465, -1.7958168118429223, 1.6015415642401545, 2.0288163340574665, -0.19026139001087655, -0.11114766738372994, 0.6011392517099333,
    -0.09141629028725604, -0.5555025428202363, -0.19026139001087655, 1.032945715913725
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 6 #2 (well-sep)');
  CmpRRel('eig[0]', ev[0], 0.10000000000000014, 1e-6);
  CmpRRel('eig[1]', ev[1], 0.6999999999999997, 1e-6);
  CmpRRel('eig[2]', ev[2], 1.499999999999999, 1e-6);
  CmpRRel('eig[3]', ev[3], 2.9000000000000004, 1e-6);
  CmpRRel('eig[4]', ev[4], 4.199999999999998, 1e-6);
  CmpRRel('eig[5]', ev[5], 6.599999999999997, 1e-6);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 16.000000000000004, 1e-9);
  // --- symmetric, order 7 #1 (well-sep) ---
  M.Init(TArrayOfDouble.Create(
    3.2520735001205017, -0.2696725535453295, 1.1041108958273742, 0.5646037366301281, 1.5689608399073425, 0.31320424819472037, -1.0411975754057115, -0.2696725535453295,
    -0.025820031670374603, 0.18342177521235087, 1.6555473386903283, 1.1892624677715125, 0.04966500873481965, -0.7042812467242934, 1.1041108958273742, 0.18342177521235087,
    -1.9189221712118987, 0.23460451276070493, -1.7873327766661153, 0.18678090684145948, 1.4373698918714344, 0.5646037366301281, 1.6555473386903283, 0.23460451276070493,
    -0.46224177005804773, 0.7870896965320685, 1.1863591834857758, -0.028151586339222028, 1.5689608399073425, 1.1892624677715125, -1.7873327766661153, 0.7870896965320685,
    0.39784201213817766, 1.060571880780074, -1.3926393759010791, 0.31320424819472037, 0.04966500873481965, 0.18678090684145948, 1.1863591834857758, 1.060571880780074,
    -0.7819444055299198, -2.005328287462617, -1.0411975754057115, -0.7042812467242934, 1.4373698918714344, -0.028151586339222028, -1.3926393759010791, -2.005328287462617,
    2.5390128662115585
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 7 #1 (well-sep)');
  CmpRRel('eig[0]', ev[0], -3.9999999999999987, 1e-6);
  CmpRRel('eig[1]', ev[1], -2.5000000000000027, 1e-6);
  CmpRRel('eig[2]', ev[2], -0.9999999999999998, 1e-6);
  CmpRRel('eig[3]', ev[3], 1.4815351382921052e-16, 1e-6);
  CmpRRel('eig[4]', ev[4], 1.5, 1e-6);
  CmpRRel('eig[5]', ev[5], 2.9999999999999973, 1e-6);
  CmpRRel('eig[6]', ev[6], 5.999999999999998, 1e-6);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 2.999999999999997, 1e-9);
  // --- symmetric, order 8 #1 (well-sep) ---
  M.Init(TArrayOfDouble.Create(
    4.440492093761296, -0.5454288725648272, 0.3853181859508925, -2.353027573744856, -2.627915472288499, 1.011306746322341, -0.7604210009826493, -0.3647430530025336,
    -0.5454288725648272, 4.6398238597801855, 0.0327703018616899, 1.6246084553179005, -1.7520692949355055, 1.2393644689067043, 1.020423820415878, -3.464091226299134,
    0.3853181859508925, 0.0327703018616899, 1.855397063999697, -1.4949978181169263, -1.1127550162283368, 2.8139822466746596, -1.3115345471815423, 1.688208744649234,
    -2.353027573744856, 1.6246084553179005, -1.4949978181169263, 1.9556867324014382, 1.488215668880256, -2.46366614306442, 2.209446798242253, 2.335094136824288,
    -2.627915472288499, -1.7520692949355055, -1.1127550162283368, 1.488215668880256, 0.17904513967639343, -0.22927594853144023, 1.0539931845162758, -0.4482863338686043,
    1.011306746322341, 1.2393644689067043, 2.8139822466746596, -2.46366614306442, -0.22927594853144023, 1.0577003626751755, -1.6104383066658008, -2.1838639094432892,
    -0.7604210009826493, 1.020423820415878, -1.3115345471815423, 2.209446798242253, 1.0539931845162758, -1.6104383066658008, 1.7573625655959244, -0.430846788827294,
    -0.3647430530025336, -3.464091226299134, 1.688208744649234, 2.335094136824288, -0.4482863338686043, -2.1838639094432892, -0.430846788827294, -2.3855078178901152
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 8 #1 (well-sep)');
  CmpRRel('eig[0]', ev[0], -5.999999999999999, 1e-6);
  CmpRRel('eig[1]', ev[1], -2.9999999999999996, 1e-6);
  CmpRRel('eig[2]', ev[2], -0.9999999999999999, 1e-6);
  CmpRRel('eig[3]', ev[3], 0.5000000000000011, 1e-6);
  CmpRRel('eig[4]', ev[4], 1.9999999999999998, 1e-6);
  CmpRRel('eig[5]', ev[5], 3.9999999999999964, 1e-6);
  CmpRRel('eig[6]', ev[6], 6.999999999999993, 1e-6);
  CmpRRel('eig[7]', ev[7], 9.999999999999995, 1e-6);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 13.499999999999996, 1e-9);
  // --- symmetric, order 9 #1 (well-sep) ---
  M.Init(TArrayOfDouble.Create(
    -2.1352440038441967, -1.7405590219081462, 0.8792406557675978, -0.9695225716054277, -0.38956707785332645, 1.9933941415576224, -0.5532805878382351, 0.46931932035332147,
    -1.274780028823254, -1.7405590219081462, 4.667718563997424, -1.5358536472297188, 0.21657323489559638, -2.931080535769775, 1.4985118224916594, -0.8958563312137127,
    -0.40206836706182497, 0.049931731120945386, 0.8792406557675978, -1.5358536472297188, 1.777795463127532, -3.2430867234553773, 1.8345414025835458, 0.6856224932698044,
    0.20998841457603953, -1.0887336462403205, 1.36199468975751, -0.9695225716054277, 0.21657323489559638, -3.2430867234553773, 3.0701994344363377, -0.22957062964641234,
    -0.3902979285241767, 1.5945413621825004, 2.5076054114227913, -2.3635386203769, -0.38956707785332645, -2.931080535769775, 1.8345414025835458, -0.22957062964641234,
    -1.544234339687136, 1.328506282132597, 1.158243195610358, -0.5559301145362009, 0.9061689779747796, 1.9933941415576224, 1.4985118224916594, 0.6856224932698044,
    -0.3902979285241767, 1.328506282132597, 0.9200409152031159, 0.5894846818593646, 1.0227245465951706, 0.9198800765008541, -0.5532805878382351, -0.8958563312137127,
    0.20998841457603953, 1.5945413621825004, 1.158243195610358, 0.5894846818593646, -3.180888566082774, 2.771460431452428, 4.183207809192897, 0.46931932035332147,
    -0.40206836706182497, -1.0887336462403205, 2.5076054114227913, -0.5559301145362009, 1.0227245465951706, 2.771460431452428, 2.3822240480441783, -1.4625222346365154,
    -1.274780028823254, 0.049931731120945386, 1.36199468975751, -2.3635386203769, 0.9061689779747796, 0.9198800765008541, 4.183207809192897, -1.4625222346365154,
    -1.457611515194473
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 9 #1 (well-sep)');
  CmpRRel('eig[0]', ev[0], -8.000000000000004, 1e-6);
  CmpRRel('eig[1]', ev[1], -4.999999999999999, 1e-6);
  CmpRRel('eig[2]', ev[2], -2.999999999999998, 1e-6);
  CmpRRel('eig[3]', ev[3], -0.9999999999999983, 1e-6);
  CmpRRel('eig[4]', ev[4], -5.001487802053936e-16, 1e-6);
  CmpRRel('eig[5]', ev[5], 2.000000000000001, 1e-6);
  CmpRRel('eig[6]', ev[6], 4.000000000000002, 1e-6);
  CmpRRel('eig[7]', ev[7], 6.500000000000005, 1e-6);
  CmpRRel('eig[8]', ev[8], 9.0, 1e-6);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 4.500000000000007, 1e-9);
  // --- symmetric, order 10 #1 (well-sep) ---
  M.Init(TArrayOfDouble.Create(
    3.5372119616224236, -2.0666971385722905, -0.7453494093768062, 2.740017630525798, 1.0240776930748223, 0.6531414288645745, -0.15100637549321372, 0.07366019052116873,
    0.8352507132149899, 4.601335885933053, -2.0666971385722905, 0.5818237434601758, -0.1576032927379089, 1.417181231577184, 2.1355865446663893, -2.3327371950176143,
    -5.413265153926185, 3.474044333611415, -0.35964769015022224, -0.4899815102322837, -0.7453494093768062, -0.1576032927379089, 2.6433903823467007, -1.8007475944525866,
    -0.3327875079905113, 0.9533285821231798, 1.8510002118478175, -0.2179751190112264, -1.5608457502039752, 1.7237970005813508, 2.740017630525798, 1.417181231577184,
    -1.8007475944525866, -1.372277712214737, 0.18747110986177318, 1.237731231636718, -1.2755142348373492, -0.2759109396436322, -2.4515456379764045, 1.7283645086718096,
    1.0240776930748223, 2.1355865446663893, -0.3327875079905113, 0.18747110986177318, 0.00849459564280706, 0.8187366467417538, -1.4829696057446315, -1.2271386372291615,
    -1.4106808232774906, 2.955268366124081, 0.6531414288645745, -2.3327371950176143, 0.9533285821231798, 1.237731231636718, 0.8187366467417538, 0.45591108517427326,
    1.4183860773031327, -1.5124776247573697, 1.0429537499603394, 2.163091450132446, -0.15100637549321372, -5.413265153926185, 1.8510002118478175, -1.2755142348373492,
    -1.4829696057446315, 1.4183860773031327, 3.771883846876254, -2.0357589624892567, -1.184484998153913, 3.3732303569583633, 0.07366019052116873, 3.474044333611415,
    -0.2179751190112264, -0.2759109396436322, -1.2271386372291615, -1.5124776247573697, -2.0357589624892567, -2.873099633475636, 0.08269406917585537, -1.2116010254373895,
    0.8352507132149899, -0.35964769015022224, -1.5608457502039752, -2.4515456379764045, -1.4106808232774906, 1.0429537499603394, -1.184484998153913, 0.08269406917585537,
    3.7796091749910596, 3.2657979927232215, 4.601335885933053, -0.4899815102322837, 1.7237970005813508, 1.7283645086718096, 2.955268366124081, 2.163091450132446,
    3.3732303569583633, -1.2116010254373895, 3.2657979927232215, -4.532947444423325
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 10 #1 (well-sep)');
  CmpRRel('eig[0]', ev[0], -10.000000000000002, 1e-6);
  CmpRRel('eig[1]', ev[1], -7.000000000000001, 1e-6);
  CmpRRel('eig[2]', ev[2], -4.000000000000001, 1e-6);
  CmpRRel('eig[3]', ev[3], -1.9999999999999996, 1e-6);
  CmpRRel('eig[4]', ev[4], -0.49999999999999944, 1e-6);
  CmpRRel('eig[5]', ev[5], 1.0000000000000004, 1e-6);
  CmpRRel('eig[6]', ev[6], 2.9999999999999982, 1e-6);
  CmpRRel('eig[7]', ev[7], 5.499999999999996, 1e-6);
  CmpRRel('eig[8]', ev[8], 8.000000000000002, 1e-6);
  CmpRRel('eig[9]', ev[9], 12.0, 1e-6);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 5.9999999999999964, 1e-9);
  // --- symmetric, order 6 #1 (clustered) ---
  M.Init(TArrayOfDouble.Create(
    5.925050384913917, 1.2444925064804988, 1.623658325214669, 1.933493081770353, 0.6126905229278468, -1.756737547541638, 1.2444925064804988, 4.182060953292831,
    0.5071489557957389, -0.30710418490074437, -0.33901022161070726, 0.5699465652864619, 1.623658325214669, 0.5071489557957389, 3.020248780456357, 0.2654730321941132,
    -0.16348519174606854, -1.4199277376151365, 1.933493081770353, -0.30710418490074437, 0.2654730321941132, 4.257386600341189, 1.223724799808677, -0.38240217218546535,
    0.6126905229278468, -0.33901022161070726, -0.16348519174606854, 1.223724799808677, 2.758927637508219, 0.2175045264737569, -1.756737547541638, 0.5699465652864619,
    -1.4199277376151365, -0.38240217218546535, 0.2175045264737569, 4.856325643487482
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 6 #1 (clustered)');
  CmpRRel('eig[0]', ev[0], 1.9999999999999987, 1e-5);
  CmpRRel('eig[1]', ev[1], 1.9999999999999993, 1e-5);
  CmpRRel('eig[2]', ev[2], 1.9999999999999996, 1e-5);
  CmpRRel('eig[3]', ev[3], 4.9999999999999964, 1e-5);
  CmpRRel('eig[4]', ev[4], 5.0, 1e-5);
  CmpRRel('eig[5]', ev[5], 9.0, 1e-5);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 24.999999999999993, 1e-8);
  // --- symmetric, order 6 #2 (clustered) ---
  M.Init(TArrayOfDouble.Create(
    3.062741796832745, -0.6669652865785141, 0.012600730538453758, -0.37761777800404034, -0.6976079886566673, 0.9416954189854538, -0.6669652865785141, 1.5922323950700792,
    -1.2631339799846288, 0.6038806167982181, 0.2997387318667559, 0.053088260834368114, 0.012600730538453758, -1.2631339799846288, 5.5031842472011006, -2.4582054425155304,
    0.4035731119675662, -0.9322854776390861, -0.37761777800404034, 0.6038806167982181, -2.4582054425155304, 4.124065238114904, -1.6707478937678157, -0.4593295127566775,
    -0.6976079886566673, 0.2997387318667559, 0.4035731119675662, -1.6707478937678157, 2.72094513633354, 0.3300346186508733, 0.9416954189854538, 0.053088260834368114,
    -0.9322854776390861, -0.4593295127566775, 0.3300346186508733, 1.9971311864476304
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 6 #2 (clustered)');
  CmpRRel('eig[0]', ev[0], 0.9999999999999999, 1e-5);
  CmpRRel('eig[1]', ev[1], 1.0001000000000007, 1e-5);
  CmpRRel('eig[2]', ev[2], 1.0002, 1e-5);
  CmpRRel('eig[3]', ev[3], 3.9999999999999996, 1e-5);
  CmpRRel('eig[4]', ev[4], 4.0, 1e-5);
  CmpRRel('eig[5]', ev[5], 8.0, 1e-5);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 19.0003, 1e-8);
  // --- symmetric, order 8 #1 (clustered) ---
  M.Init(TArrayOfDouble.Create(
    6.234978496224475, -2.1760849770907154, 0.5177758267409841, 0.1889409563020285, -0.06602061550096633, -1.930319749289369, 0.7714197854685676, -1.3871552850974478,
    -2.1760849770907154, 5.219583570499929, -0.4448344735665426, 0.0016726506529782157, 0.9260476248057818, 0.10064770336902626, -0.05026372270939565, 0.3445287836910894,
    0.5177758267409841, -0.4448344735665426, 4.024392222750218, -1.266214382507782, -0.8401029540159997, -0.9557152762733626, 0.9648564013872885, -0.18228195617170495,
    0.1889409563020285, 0.0016726506529782157, -1.266214382507782, 5.631648325652612, 0.734948996854277, 0.008278381669252158, -0.8892223223266991, 1.2201853356031882,
    -0.06602061550096633, 0.9260476248057818, -0.8401029540159997, 0.734948996854277, 4.77350539405715, -0.36989930939681526, -0.2730303542830561, -1.2769842566422986,
    -1.930319749289369, 0.10064770336902626, -0.9557152762733626, 0.008278381669252158, -0.36989930939681526, 7.460561333608639, -2.190168343860455, 0.5335550365967634,
    0.7714197854685676, -0.05026372270939565, 0.9648564013872885, -0.8892223223266991, -0.2730303542830561, -2.190168343860455, 4.410603860843832, -0.3619267166442616,
    -1.3871552850974478, 0.3445287836910894, -0.18228195617170495, 1.2201853356031882, -1.2769842566422986, 0.5335550365967634, -0.3619267166442616, 6.2447267963631585
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 8 #1 (clustered)');
  CmpRRel('eig[0]', ev[0], 2.9999999999999982, 1e-5);
  CmpRRel('eig[1]', ev[1], 2.9999999999999987, 1e-5);
  CmpRRel('eig[2]', ev[2], 3.0, 1e-5);
  CmpRRel('eig[3]', ev[3], 3.0000000000000018, 1e-5);
  CmpRRel('eig[4]', ev[4], 7.0, 1e-5);
  CmpRRel('eig[5]', ev[5], 7.000000000000003, 1e-5);
  CmpRRel('eig[6]', ev[6], 7.000000000000009, 1e-5);
  CmpRRel('eig[7]', ev[7], 10.999999999999998, 1e-5);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 44.000000000000014, 1e-8);
  // --- symmetric, order 10 #1 (clustered) ---
  M.Init(TArrayOfDouble.Create(
    2.4632029036275154, -0.8429906947748914, -0.5414650807133667, -0.01442687364489214, 0.4937673455536109, 0.8561820450313145, 0.45727095536309414, -2.269225303109905,
    0.24940046583090875, 0.9301344410516985, -0.8429906947748914, 2.642856253965738, 0.3580701180276382, 0.05741344697224389, -0.6395161475353561, -1.5327416181828615,
    -0.12656613463008237, -0.12882801474238215, -1.2433167093981974, -0.8499527814961385, -0.5414650807133667, 0.3580701180276382, 5.12925135199565, -0.30094835297882305,
    -0.7374017606314236, 1.1525256624538633, -0.6888957698191431, -0.9612044136503952, -0.5611969065639117, 1.804995863914624, -0.01442687364489214, 0.05741344697224389,
    -0.30094835297882305, 1.6895430611983289, -0.07423748058644758, 0.694765837719212, -0.6905467327581472, -0.003966909057530379, -0.2252333014147013, -1.4852249798164865,
    0.4937673455536109, -0.6395161475353561, -0.7374017606314236, -0.07423748058644758, 1.4380504067877584, 0.6182367571276228, 0.6701819464791814, -0.10319046257465185,
    0.06380088549675231, 0.4964547986708878, 0.8561820450313145, -1.5327416181828615, 1.1525256624538633, 0.694765837719212, 0.6182367571276228, 5.752185873525145,
    0.7493398913272242, -0.9958248781132066, -1.384872234711523, 0.8786295748664776, 0.45727095536309414, -0.12656613463008237, -0.6888957698191431, -0.6905467327581472,
    0.6701819464791814, 0.7493398913272242, 3.9777455321850383, -0.7017168628601058, -2.050605100625509, 2.3926427788819793, -2.269225303109905, -0.12882801474238215,
    -0.9612044136503952, -0.003966909057530379, -0.10319046257465185, -0.9958248781132066, -0.7017168628601058, 7.106457640172081, 1.3983260081749098, -1.9001851396346803,
    0.24940046583090875, -1.2433167093981974, -0.5611969065639117, -0.2252333014147013, 0.06380088549675231, -1.384872234711523, -2.050605100625509, 1.3983260081749098,
    4.898171731179211, -0.5470527256078043, 0.9301344410516985, -0.8499527814961385, 1.804995863914624, -1.4852249798164865, 0.4964547986708878, 0.8786295748664776,
    2.3926427788819793, -1.9001851396346803, -0.5470527256078043, 5.902535245363529
  ));
  ev := RealEigenvalues(M.Eigenvalues); SortAscArray(ev);
  BeginCategory('Eigenvalues sym order 10 #1 (clustered)');
  CmpRRel('eig[0]', ev[0], 0.9999999999999982, 1e-5);
  CmpRRel('eig[1]', ev[1], 0.9999999999999989, 1e-5);
  CmpRRel('eig[2]', ev[2], 1.0000000000000002, 1e-5);
  CmpRRel('eig[3]', ev[3], 1.0000000000000004, 1e-5);
  CmpRRel('eig[4]', ev[4], 1.0000000000000016, 1e-5);
  CmpRRel('eig[5]', ev[5], 5.9999999999999964, 1e-5);
  CmpRRel('eig[6]', ev[6], 5.999999999999998, 1e-5);
  CmpRRel('eig[7]', ev[7], 6.0, 1e-5);
  CmpRRel('eig[8]', ev[8], 6.000000000000004, 1e-5);
  CmpRRel('eig[9]', ev[9], 12.000000000000002, 1e-5);
  s := 0; for i := 0 to High(ev) do s := s + ev[i];
  CmpRRel('sum=trace', s, 40.99999999999999, 1e-8);
end;

procedure TestLargeRealProducts;
var
  A, B, P: TRMatrix;
  u, w: TRVector;
  i, j: integer;
begin
  // --- matrix product, order 5 ---
  A.Init(TArrayOfDouble.Create(
    1.3558487040494454, -0.5777679637967572, -0.33083337758644094, -0.5947293810985288, -0.7805181361650892, 1.1993938145972192, 0.4091665052072475, 0.21204617419477811,
    1.6147056386372305, -0.07320837299860061, 1.8457342583287062, 0.61361494065275, 1.2323497586835863, 0.14875922860718127, 0.4863270871351921, 1.7076922888633321,
    1.0812578613868715, 1.3505324440275208, -1.5496062327209938, 1.2864224332968024, -1.104327978118632, 1.7264675641597758, 1.9394922228907352, -1.5964804996369848,
    -0.5713671999295311
  ));
  B.Init(TArrayOfDouble.Create(
    -0.00014634534230806295, -0.8363134325210688, -0.2394306562828339, 1.0711872778573164, -1.0595070780430849, -1.6417304168427687, -1.4546797738795125, 1.7601526917350685,
    1.2420245757001829, 0.9124121862492349, 1.3008406768312168, -1.282658211653449, 1.4971648952131944, 0.20296954845279824, -1.610700000695224, 1.8668500938903985,
    1.7663266448536712, -1.3064396290184623, -1.66703056830623, -0.2433591097412915, 1.3351474293293175, -1.6461050202011287, 0.09872400625694855, 1.7137102824026789,
    -0.5681439961226422
  ));
  P := A * B;
  BeginCategory('Real matrix*matrix order 5 (vs numpy)');
  CmpRRel('AB[0,0]', P[0,0], -1.6343980809070637, 1e-11);
  CmpRRel('AB[0,1]', P[0,1], 0.36522750570587, 1e-11);
  CmpRRel('AB[0,2]', P[0,2], -1.1369815461428536, 1e-11);
  CmpRRel('AB[0,3]', P[0,3], 0.3214668737406409, 1e-11);
  CmpRRel('AB[0,4]', P[0,4], -0.8426410025318578, 1e-11);
  CmpRRel('AB[1,0]', P[1,0], 2.5205910679894585, 1e-11);
  CmpRRel('AB[1,1]', P[1,1], 1.1023480995492392, 1e-11);
  CmpRRel('AB[1,2]', P[1,2], -1.36625089388275, 1e-11);
  CmpRRel('AB[1,3]', P[1,3], -0.9812124334048552, 1e-11);
  CmpRRel('AB[1,4]', P[1,4], -1.5903409324216322, 1e-11);
  CmpRRel('AB[2,0]', P[2,0], 1.5224598073622546, 1e-11);
  CmpRRel('AB[2,1]', P[2,1], -4.5546972042633795, 1e-11);
  CmpRRel('AB[2,2]', P[2,2], 2.3368286289675773, 1e-11);
  CmpRRel('AB[2,3]', P[2,3], 3.574818914619263, 1e-11);
  CmpRRel('AB[2,4]', P[2,4], -3.6931502466854265, 1e-11);
  CmpRRel('AB[3,0]', P[3,0], -1.1938752299310247, 1e-11);
  CmpRRel('AB[3,1]', P[3,1], -9.588018673968683, 1e-11);
  CmpRRel('AB[3,2]', P[3,2], 5.667742582927191, 1e-11);
  CmpRRel('AB[3,3]', P[3,3], 8.234120361410039, 1e-11);
  CmpRRel('AB[3,4]', P[3,4], -3.3518242152077, 1e-11);
  CmpRRel('AB[4,0]', P[4,0], -4.054511543414003, 1e-11);
  CmpRRel('AB[4,1]', P[4,1], -5.954974378277458, 1e-11);
  CmpRRel('AB[4,2]', P[4,2], 8.236293906112662, 1e-11);
  CmpRRel('AB[4,3]', P[4,3], 3.0372548728537407, 1e-11);
  CmpRRel('AB[4,4]', P[4,4], 0.3344901465749805, 1e-11);
  u.Init(TArrayOfDouble.Create(1.424954195784542, -1.7977301165340287, 1.910423984247195, -1.6741566519831728, 0.19931017979268129));
  BeginCategory('Real matrix*vector order 5 (vs numpy)');
  w := A * u;
  CmpRRel('Au[0]', w[0], 3.1787660886850437, 1e-11);
  CmpRRel('Au[1]', w[1], -1.3392529635340868, 1e-11);
  CmpRRel('Au[2]', w[2], 3.729166939970804, 1e-11);
  CmpRRel('Au[3]', w[3], 5.920343712478677, 1e-11);
  CmpRRel('Au[4]', w[4], 1.5867920875028703, 1e-11);
  w := u * A;
  BeginCategory('Real vector*matrix order 5 (vs numpy)');
  CmpRRel('uA[0]', w[0], 0.22292270044627702, 1e-11);
  CmpRRel('uA[1]', w[1], -1.8526916141057068, 1e-11);
  CmpRRel('uA[2]', w[2], -0.37275599820968414, 1e-11);
  CmpRRel('uA[3]', w[3], -1.1899851176126044, 1e-11);
  CmpRRel('uA[4]', w[4], -2.3190547379500037, 1e-11);
  // --- matrix product, order 6 ---
  A.Init(TArrayOfDouble.Create(
    0.6203045512449648, -1.1567275206419465, 0.4868410472493241, 0.8838748568447024, -1.6123830780717783, 1.0754412780834972, 1.9133504776627883, 1.9368413686450974,
    1.4607312165713364, -1.5837222537744298, 0.5528199793058337, 0.5011911169313752, -1.465989277644602, 1.1293172499549264, 0.9603097115639856, 0.01946917325047348,
    -0.45378692629771056, 1.5579969604587753, 0.030729062327908174, -0.8209594812499428, -0.1682618531882496, 0.2301721489158406, -0.07537106382295011, 1.527843602905461,
    0.3757351723598621, -0.5976901242179271, -1.870935934727469, -0.9113991199840847, 0.10150497680087778, -0.7701884114756421, -0.9861588960800529, 1.0756739618683655,
    0.24247412137973345, 0.46923767265645866, 1.579736565003199, -0.21295820474820504
  ));
  B.Init(TArrayOfDouble.Create(
    -1.364683497343325, -0.4932600835011254, 1.3472809043693528, 0.5661806262479132, -0.1775783931713235, 0.1679780656861447, 1.310041635965408, -1.7449387659873303,
    -1.574098603048812, 0.602983393160855, 0.2643908574176139, 0.23155923004182766, 0.005626960489368393, 0.25123182639219754, -0.40052930062621916, 1.2903467252811276,
    0.35194487594853197, 1.8500623266125125, -0.2410236256484546, 0.9460013057165715, -1.717348137636654, 0.903537178408949, -1.7808169212511231, -0.6193290195631387,
    0.7813948226290104, 1.9630615486386538, -0.5397561331278222, -1.1872359229454448, 1.0337600943835201, -0.6433996701185856, -1.6779387238397532, -0.8982090762442727,
    -0.8970982208924054, -0.6885820164309169, 1.4084916088240078, -0.6531925030089019
  ));
  P := A * B;
  BeginCategory('Real matrix*matrix order 6 (vs numpy)');
  CmpRRel('AB[0,0]', P[0,0], -5.636608240209622, 1e-11);
  CmpRRel('AB[0,1]', P[0,1], -1.4602743873430857, 1e-11);
  CmpRRel('AB[0,2]', P[0,2], 0.8491299057571224, 1e-11);
  CmpRRel('AB[0,3]', P[0,3], 2.254274067220145, 1e-11);
  CmpRRel('AB[0,4]', P[0,4], -1.970726222830958, 1e-11);
  CmpRRel('AB[0,5]', P[0,5], 0.524560116930537, 1e-11);
  CmpRRel('AB[1,0]', P[1,0], -0.09283834331718058, 1e-11);
  CmpRRel('AB[1,1]', P[1,1], -4.81962491811761, 1e-11);
  CmpRRel('AB[1,2]', P[1,2], 0.9157724460145615, 1e-11);
  CmpRRel('AB[1,3]', P[1,3], 1.7036440291470156, 1e-11);
  CmpRRel('AB[1,4]', P[1,4], 4.784136418234125, 1e-11);
  CmpRRel('AB[1,5]', P[1,5], 3.77012487955544, 1e-11);
  CmpRRel('AB[2,0]', P[2,0], 0.511964899853903, 1e-11);
  CmpRRel('AB[2,1]', P[2,1], -3.278015905922289, 1e-11);
  CmpRRel('AB[2,2]', P[2,2], -5.323565615695015, 1e-11);
  CmpRRel('AB[2,3]', P[2,3], 0.5736118851179969, 1e-11);
  CmpRRel('AB[2,4]', P[2,4], 2.587533055103913, 1e-11);
  CmpRRel('AB[2,5]', P[2,5], 1.0541192093626306, 1e-11);
  CmpRRel('AB[3,0]', P[3,0], -3.7963727791136823, 1e-11);
  CmpRRel('AB[3,1]', P[3,1], 0.07255599653119453, 1e-11);
  CmpRRel('AB[3,2]', P[3,2], -0.3241638414276314, 1e-11);
  CmpRRel('AB[3,3]', P[3,3], -1.4493361655287875, 1e-11);
  CmpRRel('AB[3,4]', P[3,4], 1.3824149429344679, 1e-11);
  CmpRRel('AB[3,5]', P[3,5], -1.588268413419742, 1e-11);
  CmpRRel('AB[4,0]', P[4,0], 0.28502692411763847, 1e-11);
  CmpRRel('AB[4,1]', P[4,1], 0.416424834383697, 1e-11);
  CmpRRel('AB[4,2]', P[4,2], 4.397744975124563, 1e-11);
  CmpRRel('AB[4,3]', P[4,3], -2.9754747552409024, 1e-11);
  CmpRRel('AB[4,4]', P[4,4], -0.2400497135307631, 1e-11);
  CmpRRel('AB[4,5]', P[4,5], -2.534404534747509, 1e-11);
  CmpRRel('AB[5,0]', P[5,0], 4.234968266474025, 1e-11);
  CmpRRel('AB[5,1]', P[5,1], 2.406665391291836, 1e-11);
  CmpRRel('AB[5,2]', P[5,2], -4.58644043666, 1e-11);
  CmpRRel('AB[5,3]', P[5,3], -0.9017619633786438, 1e-11);
  CmpRRel('AB[5,4]', P[5,4], 1.0423487863930165, 1e-11);
  CmpRRel('AB[5,5]', P[5,5], -0.6358903820432925, 1e-11);
  u.Init(TArrayOfDouble.Create(-0.9978385936001821, -0.9465489884754725, -0.021353216666806674, 1.9213123665494587, -1.4501551569676479, -1.1181019199046665));
  BeginCategory('Real matrix*vector order 6 (vs numpy)');
  w := A * u;
  CmpRRel('Au[0]', w[0], 3.2994921920642293, 1e-11);
  CmpRRel('Au[1]', w[1], -8.178604143484787, 1e-11);
  CmpRRel('Au[2]', w[2], -0.6731707009400056, 1e-11);
  CmpRRel('Au[3]', w[3], -0.4067438786579649, 1e-11);
  CmpRRel('Au[4]', w[4], -0.8063607971636277, 1e-11);
  CmpRRel('Au[5]', w[5], -1.190530302530579, 1e-11);
  w := u * A;
  BeginCategory('Real vector*matrix order 6 (vs numpy)');
  CmpRRel('uA[0]', w[0], -1.7819482089226109, 1e-11);
  CmpRRel('uA[1]', w[1], -2.6164917416910916, 1e-11);
  CmpRRel('uA[2]', w[2], 0.22980489139762028, 1e-11);
  CmpRRel('uA[3]', w[3], 1.855937711148242, 1e-11);
  CmpRRel('uA[4]', w[4], -0.9629991275376699, 1e-11);
  CmpRRel('uA[5]', w[5], 2.7096794788926775, 1e-11);
  // --- matrix product, order 7 ---
  A.Init(TArrayOfDouble.Create(
    0.5373998529336239, -0.4115961189684323, -0.03258735795364753, -0.6215403050438351, -1.7850752548612308, -0.819909640538925, -1.3439304158633534, 0.1310483993327427,
    1.9464828546550974, -0.7091661311084407, -1.2831479751443071, 1.1717018638009127, -0.3125314388970266, -1.7564287205127878, 1.60985552538685, -0.803369737673501,
    0.47392506755082, 1.7545520138442643, 1.256790950595176, 1.6982206245365323, -0.8415766185083586, -1.4643325803172136, -0.41277327975010225, 0.3138237926674887,
    -0.05835103218657878, 1.0872423162774272, 0.28640948281544487, -0.9842500850716114, -1.0532080206330812, 1.4537498409821925, -1.2771549512782552, 0.6304973828873885,
    -0.1587017906070427, 0.24109219828925843, -0.05911871300647942, -0.26856724713175506, 0.7833953476803397, 0.18737528673634163, 0.6160133879262388, -0.839063111693692,
    -0.4739319563843871, 0.9624611424825678, -1.3631708271908192, 0.03161746541379662, -1.4268518214296684, 0.3796095276420699, -1.0288993713732424, 0.6032068789596456,
    -1.6730678529067795
  ));
  B.Init(TArrayOfDouble.Create(
    -1.8024253854372367, 0.06290293718821172, 1.0169055089201282, 0.22976391342389846, 1.362762194499298, 0.9373000638141256, -0.8658562963948127, 0.45695925514318425,
    0.7917315106346501, 1.5552022846319442, 1.605279965897381, 0.3391444419013907, 0.3170481862600618, -1.6515137933618185, 0.46742521969403317, 0.9046115818956677,
    0.31919856896240795, -1.23134460840771, -0.654462563976943, -0.40485754927161866, 1.7681891024471965, -1.5879921913740604, 0.8518342905255158, 0.49881004656100725,
    -1.3536422264688421, -0.2965146774963223, -1.363640641799559, 0.08413124081602241, -0.7586094165620247, -1.8805267766446416, -1.6796109079918082, 0.8636206143514089,
    1.5861578655462774, 1.7832153869488154, 1.1677890099393378, 1.0407019718633754, 1.0110444569809731, -0.5626324985336777, 0.20358852768164803, -1.9711481631997545,
    -0.23266582470672414, -0.014537147295520914, -0.03926090870420307, 1.4626555005370454, 1.9593280936598116, 0.2825407298546101, -1.079574705406015, 1.4750451156581112,
    -1.890433054890392
  ));
  P := A * B;
  BeginCategory('Real matrix*matrix order 7 (vs numpy)');
  CmpRRel('AB[0,0]', P[0,0], 0.36872045239463613, 1e-11);
  CmpRRel('AB[0,1]', P[0,1], -0.2887883313578879, 1e-11);
  CmpRRel('AB[0,2]', P[0,2], 0.4122763194066565, 1e-11);
  CmpRRel('AB[0,3]', P[0,3], -1.7440495060474508, 1e-11);
  CmpRRel('AB[0,4]', P[0,4], 1.034006204431484, 1e-11);
  CmpRRel('AB[0,5]', P[0,5], -3.740806732522371, 1e-11);
  CmpRRel('AB[0,6]', P[0,6], 0.5724724174272394, 1e-11);
  CmpRRel('AB[1,0]', P[1,0], 1.2142480596369987, 1e-11);
  CmpRRel('AB[1,1]', P[1,1], -5.27366423797322, 1e-11);
  CmpRRel('AB[1,2]', P[1,2], -2.9395565172887164, 1e-11);
  CmpRRel('AB[1,3]', P[1,3], 6.216926712910141, 1e-11);
  CmpRRel('AB[1,4]', P[1,4], 6.054067455735107, 1e-11);
  CmpRRel('AB[1,5]', P[1,5], 2.348125094158746, 1e-11);
  CmpRRel('AB[1,6]', P[1,6], 0.0032497660001442625, 1e-11);
  CmpRRel('AB[2,0]', P[2,0], -4.986472905671758, 1e-11);
  CmpRRel('AB[2,1]', P[2,1], -0.4888719763056878, 1e-11);
  CmpRRel('AB[2,2]', P[2,2], -3.301185738365714, 1e-11);
  CmpRRel('AB[2,3]', P[2,3], -2.684998273258667, 1e-11);
  CmpRRel('AB[2,4]', P[2,4], 0.6455444447749948, 1e-11);
  CmpRRel('AB[2,5]', P[2,5], -0.7255922286105386, 1e-11);
  CmpRRel('AB[2,6]', P[2,6], 3.952398072204113, 1e-11);
  CmpRRel('AB[3,0]', P[3,0], 2.2019969907784005, 1e-11);
  CmpRRel('AB[3,1]', P[3,1], -3.379367574498713, 1e-11);
  CmpRRel('AB[3,2]', P[3,2], -5.975723970912138, 1e-11);
  CmpRRel('AB[3,3]', P[3,3], -0.5873224499565441, 1e-11);
  CmpRRel('AB[3,4]', P[3,4], -0.10105690933774401, 1e-11);
  CmpRRel('AB[3,5]', P[3,5], -1.1305358846459896, 1e-11);
  CmpRRel('AB[3,6]', P[3,6], 5.625757961614064, 1e-11);
  CmpRRel('AB[4,0]', P[4,0], 1.338052815977723, 1e-11);
  CmpRRel('AB[4,1]', P[4,1], 0.9222073964221856, 1e-11);
  CmpRRel('AB[4,2]', P[4,2], 1.1117724293479176, 1e-11);
  CmpRRel('AB[4,3]', P[4,3], 2.7061583043499544, 1e-11);
  CmpRRel('AB[4,4]', P[4,4], -0.9564739892071221, 1e-11);
  CmpRRel('AB[4,5]', P[4,5], -1.2952653857848875, 1e-11);
  CmpRRel('AB[4,6]', P[4,6], -3.771243088730994, 1e-11);
  CmpRRel('AB[5,0]', P[5,0], 0.056923820281153566, 1e-11);
  CmpRRel('AB[5,1]', P[5,1], 3.804051750618531, 1e-11);
  CmpRRel('AB[5,2]', P[5,2], 4.874040542629347, 1e-11);
  CmpRRel('AB[5,3]', P[5,3], -0.4179083233171089, 1e-11);
  CmpRRel('AB[5,4]', P[5,4], -1.841341388086746, 1e-11);
  CmpRRel('AB[5,5]', P[5,5], -0.8855240894960171, 1e-11);
  CmpRRel('AB[5,6]', P[5,6], -3.470534089274472, 1e-11);
  CmpRRel('AB[6,0]', P[6,0], 2.6756756097140713, 1e-11);
  CmpRRel('AB[6,1]', P[6,1], -0.9304772840242225, 1e-11);
  CmpRRel('AB[6,2]', P[6,2], -3.492462415544066, 1e-11);
  CmpRRel('AB[6,3]', P[6,3], -0.25784428808435716, 1e-11);
  CmpRRel('AB[6,4]', P[6,4], -2.0404987032971134, 1e-11);
  CmpRRel('AB[6,5]', P[6,5], -5.6506004727726165, 1e-11);
  CmpRRel('AB[6,6]', P[6,6], 0.5896030294073081, 1e-11);
  u.Init(TArrayOfDouble.Create(-0.701614436224661, -1.66823687693759, -1.8471983871387563, -1.2278061957446877, -0.32865101494560234, 1.1786932958954432, -0.28409487131106514));
  BeginCategory('Real matrix*vector order 7 (vs numpy)');
  w := A * u;
  CmpRRel('Au[0]', w[0], 1.13496721798212, 1e-11);
  CmpRRel('Au[1]', w[1], -0.7081796869571625, 1e-11);
  CmpRRel('Au[2]', w[2], -0.9912470636650819, 1e-11);
  CmpRRel('Au[3]', w[3], 1.4678354878592745, 1e-11);
  CmpRRel('Au[4]', w[4], 0.2519034156092277, 1e-11);
  CmpRRel('Au[5]', w[5], -2.7772144831983034, 1e-11);
  CmpRRel('Au[6]', w[6], 4.5977210861797175, 1e-11);
  w := u * A;
  BeginCategory('Real vector*matrix order 7 (vs numpy)');
  CmpRRel('uA[0]', w[0], -1.3546238705860985, 1e-11);
  CmpRRel('uA[1]', w[1], -0.5309997016157053, 1e-11);
  CmpRRel('uA[2]', w[2], 0.991129985017251, 1e-11);
  CmpRRel('uA[3]', w[3], -0.1816533440395548, 1e-11);
  CmpRRel('uA[4]', w[4], -5.003242284049874, 1e-11);
  CmpRRel('uA[5]', w[5], -3.2011924204143294, 1e-11);
  CmpRRel('uA[6]', w[6], 8.26527338690053, 1e-11);
  // --- matrix product, order 8 ---
  A.Init(TArrayOfDouble.Create(
    0.7867892889901378, -1.4651139088060887, 0.3820827243948717, 1.6767096642265646, 0.7054678854683609, 0.16364307982053639, -1.0024075528722212, 1.9933619139561802,
    0.1353146904349316, 1.4385096719506612, 0.23354300810601147, -1.470912362310251, 0.05814087372759902, 0.44307542419613943, -0.6073770901995275, 0.22684685507142532,
    1.3741257027591542, 1.93165910059616, -1.5912755204527147, 1.6394475758430431, -0.9039176107131439, 1.9091613408443902, 0.38346121459227733, -0.1255524906811556,
    0.8007726000409501, 0.7603745562244182, -1.8433239251654507, -1.4448256979585237, -1.7285845520711973, -0.3098761433598578, 1.0105331399769253, -1.7933108415423211,
    -0.6143498971577253, 0.17162384808693165, 0.24817072050140343, -1.6616690413347603, -1.2847564651557306, 1.143947758061659, 0.6423236249869304, -0.2469304196108859,
    1.9942179945758616, 0.6444504516696088, 1.2001997734902172, 1.297327169384856, -0.7380989104806943, 0.503611478947231, 1.230465800427114, -1.1914734782108245,
    -0.21038189362673654, -0.67637492528833, -0.38226175878112967, -0.23662627171695627, -1.0953140015774832, 0.7085627325982204, 1.7047738933184728, 0.6562982541872655,
    -1.5534668374955785, 1.804626878061788, 0.33817571635233623, 0.18679957236241496, -1.1695559241999005, 0.4683967580204045, 0.8573374097408912, 0.5615451122151369
  ));
  B.Init(TArrayOfDouble.Create(
    -0.30162193701584483, 1.1781078027639786, 0.8633567243840115, -0.08654728576283688, -1.3774519150125544, -1.1327618933945391, -0.3638953049063396, -0.9628284933136468,
    -0.298741355779351, 1.9243745945614652, 0.08153293020065888, -0.802684546117018, -1.274396674383023, 1.2966798426727508, 1.3609109939840254, -1.7848452042049634,
    1.2443993604772978, 1.2759548699563283, 1.5342603167479747, 1.0618913701115473, 1.0610128183808625, -1.0846912347707058, 1.2815711276150061, -1.2512045273595356,
    -1.9734416432240987, -1.508381516932424, 0.031029730445019332, 1.2531166940641176, -1.907458534988283, 1.592256270387546, -1.810090879892857, 1.9404127073079454,
    0.7773260600549405, 1.8562929796161036, -0.26602863674785393, 1.1278091674149353, 1.9976830860799337, 0.1643581608837663, -0.2958010479324371, -1.6194069683260945,
    -0.8421146863111826, -0.4285485204747159, 0.4619969445605854, -1.1699231165442678, -1.888420833448683, 1.6406988294678393, -0.44847355744148665, -0.649669065091715,
    0.7859279799403169, -0.6072942902336274, -0.7986247814007403, -1.559571123756128, 0.5509920538989341, -1.419901868525455, 1.044594213859761, -0.5998536476962095,
    1.6856423107738414, 0.5330017049653626, 1.7469668016375524, 1.6817586660236543, -0.7894143099374369, 1.7382416880658678, 1.9020901119013418, 1.0940948556750367
  ));
  P := A * B;
  BeginCategory('Real matrix*matrix order 8 (vs numpy)');
  CmpRRel('AB[0,0]', P[0,0], 0.34979939984501984, 1e-11);
  CmpRRel('AB[0,1]', P[0,1], -1.0234551721236351, 1e-11);
  CmpRRel('AB[0,2]', P[0,2], 5.368879547081963, 1e-11);
  CmpRRel('AB[0,3]', P[0,3], 9.134635908130317, 1e-11);
  CmpRRel('AB[0,4]', P[0,4], -3.0351203660897403, 1e-11);
  CmpRRel('AB[0,5]', P[0,5], 4.736984646227267, 1e-11);
  CmpRRel('AB[0,6]', P[0,6], -2.363152115291873, 1e-11);
  CmpRRel('AB[0,7]', P[0,7], 6.16637487870854, 1e-11);
  CmpRRel('AB[1,0]', P[1,0], 2.299926372366848, 1e-11);
  CmpRRel('AB[1,1]', P[1,1], 5.852147695883406, 1e-11);
  CmpRRel('AB[1,2]', P[1,2], 1.6173771876168352, 1e-11);
  CmpRRel('AB[1,3]', P[1,3], -1.8856510694196724, 1e-11);
  CmpRRel('AB[1,4]', P[1,4], -0.2004268822819451, 1e-11);
  CmpRRel('AB[1,5]', P[1,5], 1.1098554667416813, 1e-11);
  CmpRRel('AB[1,6]', P[1,6], 4.451345098173746, 1e-11);
  CmpRRel('AB[1,7]', P[1,7], -5.613665829355486, 1e-11);
  CmpRRel('AB[2,0]', P[2,0], -8.42770448418885, 1e-11);
  CmpRRel('AB[2,1]', P[2,1], -1.9631019408336565, 1e-11);
  CmpRRel('AB[2,2]', P[2,2], -0.44978776134814796, 1e-11);
  CmpRRel('AB[2,3]', P[2,3], -5.3669849550112705, 1e-11);
  CmpRRel('AB[2,4]', P[2,4], -14.270677968144721, 1e-11);
  CmpRRel('AB[2,5]', P[2,5], 7.505724150239136, 1e-11);
  CmpRRel('AB[2,6]', P[2,6], -3.3051830399544504, 1e-11);
  CmpRRel('AB[2,7]', P[2,7], 0.2575566544000861, 1e-11);
  CmpRRel('AB[3,0]', P[3,0], -3.2226347522177883, 1e-11);
  CmpRRel('AB[3,1]', P[3,1], -2.411498982525289, 1e-11);
  CmpRRel('AB[3,2]', P[3,2], -5.742823479411313, 1e-11);
  CmpRRel('AB[3,3]', P[3,3], -10.626487165606768, 1e-11);
  CmpRRel('AB[3,4]', P[3,4], -2.167416347855057, 1e-11);
  CmpRRel('AB[3,5]', P[3,5], -5.566803761075354, 1e-11);
  CmpRRel('AB[3,6]', P[3,6], -0.7088335687738242, 1e-11);
  CmpRRel('AB[3,7]', P[3,7], -2.192965802842563, 1e-11);
  CmpRRel('AB[4,0]', P[4,0], 1.8486344915466382, 1e-11);
  CmpRRel('AB[4,1]', P[4,1], -0.9672316563767068, 1e-11);
  CmpRRel('AB[4,2]', P[4,2], -0.2612852038348802, 1e-11);
  CmpRRel('AB[4,3]', P[4,3], -6.1076421600831425, 1e-11);
  CmpRRel('AB[4,4]', P[4,4], -0.11754772677548916, 1e-11);
  CmpRRel('AB[4,5]', P[4,5], -1.6720859454899848, 1e-11);
  CmpRRel('AB[4,6]', P[4,6], 3.85122985283428, 1e-11);
  CmpRRel('AB[4,7]', P[4,7], -2.56775383800731, 1e-11);
  CmpRRel('AB[5,0]', P[5,0], -3.8998782731469883, 1e-11);
  CmpRRel('AB[5,1]', P[5,1], 0.19584224844071696, 1e-11);
  CmpRRel('AB[5,2]', P[5,2], 1.0208173653057788, 1e-11);
  CmpRRel('AB[5,3]', P[5,3], -3.1340916645555827, 1e-11);
  CmpRRel('AB[5,4]', P[5,4], -5.576370371506685, 1e-11);
  CmpRRel('AB[5,5]', P[5,5], -3.772744469872997, 1e-11);
  CmpRRel('AB[5,6]', P[5,6], -1.6472640272492407, 1e-11);
  CmpRRel('AB[5,7]', P[5,7], -3.2282621520694055, 1e-11);
  CmpRRel('AB[6,0]', P[6,0], 1.2548052141023445, 1e-11);
  CmpRRel('AB[6,1]', P[6,1], -4.702645896527607, 1e-11);
  CmpRRel('AB[6,2]', P[6,2], -0.4268176563710015, 1e-11);
  CmpRRel('AB[6,3]', P[6,3], -3.7605670649816045, 1e-11);
  CmpRRel('AB[6,4]', P[6,4], -1.9073981722422306, 1e-11);
  CmpRRel('AB[6,5]', P[6,5], -0.89815522550618, 1e-11);
  CmpRRel('AB[6,6]', P[6,6], 2.1298490793510174, 1e-11);
  CmpRRel('AB[6,7]', P[6,7], 2.437786785911591, 1e-11);
  CmpRRel('AB[7,0]', P[7,0], 0.2984301581098659, 1e-11);
  CmpRRel('AB[7,1]', P[7,1], -0.8007621564445552, 1e-11);
  CmpRRel('AB[7,2]', P[7,2], 0.1544294054762005, 1e-11);
  CmpRRel('AB[7,3]', P[7,3], -2.9806296276288564, 1e-11);
  CmpRRel('AB[7,4]', P[7,4], -3.3493262215520767, 1e-11);
  CmpRRel('AB[7,5]', P[7,5], 4.365385973147889, 1e-11);
  CmpRRel('AB[7,6]', P[7,6], 5.216079289224521, 1e-11);
  CmpRRel('AB[7,7]', P[7,7], -0.09612544195492402, 1e-11);
  u.Init(TArrayOfDouble.Create(0.9070925915762484, -1.7460064414572276, 0.3285849551813822, 1.3473516677568789, -1.7461546500740175, -1.9228393747001915, -1.4973236931843341, -1.6694871356591952));
  BeginCategory('Real matrix*vector order 8 (vs numpy)');
  w := A * u;
  CmpRRel('Au[0]', w[0], 2.282974375844187, 1e-11);
  CmpRRel('Au[1]', w[1], -4.716765332809426, 1e-11);
  CmpRRel('Au[2]', w[2], -2.8973747111146784, 1e-11);
  CmpRRel('Au[3]', w[3], 1.9414113671841773, 1e-11);
  CmpRRel('Au[4]', w[4], -3.51999951304225, 1e-11);
  CmpRRel('Au[5]', w[5], 3.2932641543298766, 1e-11);
  CmpRRel('Au[6]', w[6], -2.5524496644750068, 1e-11);
  CmpRRel('Au[7]', w[7], -5.276854416448588, 1e-11);
  w := u * A;
  BeginCategory('Real vector*matrix order 8 (vs numpy)');
  CmpRRel('uA[0]', w[0], 2.154561487469883, 1e-11);
  CmpRRel('uA[1]', w[1], -5.720340720766944, 1e-11);
  CmpRRel('uA[2]', w[2], -5.801004207409791, 1e-11);
  CmpRRel('uA[3]', w[3], 3.1305888688403427, 1e-11);
  CmpRRel('uA[4]', w[4], 5.167612645031397, 1e-11);
  CmpRRel('uA[5]', w[5], -5.224167372246652, 1e-11);
  CmpRRel('uA[6]', w[6], -5.832745648962656, 1e-11);
  CmpRRel('uA[7]', w[7], -0.24337970684357413, 1e-11);
end;

procedure TestComplexEigenvalues;
var
  A: TRMatrix;
  ev: TArrayOfComplex;
  i, j: integer;
  tmp: TComplex;
begin
  Section('TRMatrix - complex eigenvalues');

  // Rotation by 90 degrees: [[0,-1],[1,0]] has eigenvalues +i and -i.
  A.Init(TArrayOfDouble.Create(0,-1, 1,0));
  ev := A.Eigenvalues;
  if ev[0].Im > ev[1].Im then begin tmp := ev[0]; ev[0] := ev[1]; ev[1] := tmp; end;
  CheckCplxNear('rot90 eig[0] = -i', ev[0], C(0,-1), EPS);
  CheckCplxNear('rot90 eig[1] = +i', ev[1], C(0, 1), EPS);

  // Block upper triangular: [[1,-2,5],[2,1,-1],[0,0,3]]
  // eigenvalues: 1+2i, 1-2i (from the 2x2 block) and 3.
  A.Init(TArrayOfDouble.Create(1,-2,5, 2,1,-1, 0,0,3));
  ev := A.Eigenvalues;
  for i := 0 to 1 do
    for j := i+1 to 2 do
      if (ev[i].Re > ev[j].Re) or
         ((ev[i].Re = ev[j].Re) and (ev[i].Im > ev[j].Im)) then
      begin tmp := ev[i]; ev[i] := ev[j]; ev[j] := tmp; end;
  CheckCplxNear('block eig[0] = 1-2i', ev[0], C(1,-2), EPS);
  CheckCplxNear('block eig[1] = 1+2i', ev[1], C(1, 2), EPS);
  CheckCplxNear('block eig[2] = 3',    ev[2], C(3, 0), EPS);
end;

procedure TestExceptions;
var
  A, B, R: TRMatrix;
  raised: boolean;
begin
  Section('TRMatrix - exceptions');

  raised := False;
  try
    A.Init(TArrayOfDouble.Create(1,2,3,4,5));
  except
    on EDimensionError do raised := True;
  end;
  Check('Init with 5 values raises EDimensionError', raised);

  raised := False;
  A.Init(2);
  B.Init(3);
  try
    R := A + B;
  except
    on EDimensionError do raised := True;
  end;
  Check('2x2 + 3x3 raises EDimensionError', raised);

  raised := False;
  try
    R := A * B;
  except
    on EDimensionError do raised := True;
  end;
  Check('2x2 * 3x3 raises EDimensionError', raised);

  raised := False;
  A.Init(TArrayOfDouble.Create(1,2, 2,4));
  try
    R := A.Inverse;
  except
    on EZeroDivide do raised := True;
  end;
  Check('singular Inverse raises EZeroDivide', raised);
end;

procedure TestSolveLinear;
var
  A: TRMatrix;
  xt, b, x, r: TRVector;
  i, j, n: integer;
  raised: boolean;
begin
  Section('TRMatrix - SolveLinear');

  // Known system: [[2,1],[1,3]] * [1,2] = [4,7]
  A.Init(TArrayOfDouble.Create(2,1, 1,3));
  b.Init(TArrayOfDouble.Create(4, 7));
  x := A.SolveLinear(b);
  CheckNear('2x2 known x[0]=1', x[0], 1.0, EPS);
  CheckNear('2x2 known x[1]=2', x[1], 2.0, EPS);

  // Random well-conditioned systems: construct b = A*xt, recover xt
  BeginCategory('SolveLinear forward error (random)');
  for n := 4 to 8 do
  begin
    A.Init(n);
    xt.Init(n);
    for i := 0 to n-1 do
    begin
      xt[i] := i + 1;
      for j := 0 to n-1 do A[i,j] := Sin(7.0*i + 13.0*j) ;
      A[i,i] := A[i,i] + n;
    end;
    b := A * xt;
    x := A.SolveLinear(b);
    for i := 0 to n-1 do
      CmpRRel(Format('n=%d x[%d]', [n, i]), x[i], xt[i], 1e-10);
  end;

  // Hilbert 8x8: residual must stay at machine level despite cond ~ 1e10
  BeginCategory('SolveLinear residual (Hilbert 8, ill-cond)');
  A.Init(8);
  for i := 0 to 7 do
    for j := 0 to 7 do
      A[i,j] := 1.0 / (i + j + 1);
  xt.Init(8);
  for i := 0 to 7 do xt[i] := 1.0;
  b := A * xt;
  x := A.SolveLinear(b);
  r := A * x - b;
  CmpRAbs('residual |Ax-b|', r.Norm, 0.0, 1e-13);

  Section('TRMatrix - SolveLinear exceptions');
  raised := False;
  A.Init(3);
  b.Init(2);
  try
    x := A.SolveLinear(b);
  except
    on EDimensionError do raised := True;
  end;
  Check('size mismatch raises EDimensionError', raised);

  raised := False;
  A.Init(TArrayOfDouble.Create(1,2, 2,4));
  b.Init(TArrayOfDouble.Create(1, 1));
  try
    x := A.SolveLinear(b);
  except
    on EZeroDivide do raised := True;
  end;
  Check('singular system raises EZeroDivide', raised);
end;

procedure TestEigenvectors;
var
  A: TRMatrix;
  V, ZA: TCMatrix;
  ev: TArrayOfComplex;
  col, res: TCVector;
  proj: TComplex;
  i, j, k, n: integer;
  offdiag: double;

  procedure LoadComplexCopy;
  var p, q: integer;
  begin
    ZA.Init(A.Order);
    for p := 0 to A.Order-1 do
      for q := 0 to A.Order-1 do
        ZA[p,q] := C(A[p,q], 0);
  end;

  procedure CheckResiduals(const tag: string; ATol: double);
  var p, q: integer;
  begin
    LoadComplexCopy;
    for q := 0 to A.Order-1 do
    begin
      col.Init(A.Order);
      for p := 0 to A.Order-1 do col[p] := V[p,q];
      res := ZA * col - ev[q] * col;
      CmpRAbs(Format('%s |Av-lv| col %d', [tag, q]), res.Norm, 0.0, ATol);
      CmpRAbs(Format('%s |v|=1 col %d',  [tag, q]), col.Norm, 1.0, 1e-9);
    end;
  end;

begin
  Section('TRMatrix - Eigenvectors');

  // Symmetric 3x3: residuals, unit norm, real vectors, orthogonality
  A.Init(TArrayOfDouble.Create(4,2,1, 2,5,3, 1,3,6));
  BeginCategory('Eigenvectors sym 3x3');
  V := A.Eigenvectors(ev);
  CheckResiduals('sym3', 1e-9);
  for j := 0 to 2 do
    for k := j+1 to 2 do
    begin
      proj := 0;
      for i := 0 to 2 do proj := proj + V[i,j] * V[i,k].Conjugate;
      CmpRAbs(Format('sym3 ortho %d.%d', [j, k]), Abs(proj), 0.0, 1e-9);
    end;
  for j := 0 to 2 do
    for i := 0 to 2 do
      CmpRAbs(Format('sym3 Im=0 [%d,%d]', [i, j]), Abs(V[i,j].Im), 0.0, 1e-12);

  // Degenerate spectrum 2,2,2,5,5: orthonormality within clusters is the
  // critical property
  A.Init(TArrayOfDouble.Create(
    3.9681451856646284, -0.6470635423949582, -1.0683766715308218, 0.6550202348522072, -0.20412897125396062,
    -0.6470635423949582, 2.39008949997781, 0.07219196833991376, -0.2941787990675794, 0.712498847669345,
    -1.0683766715308218, 0.07219196833991376, 3.019026569770013, -0.23153568981902073, -0.904663136857451,
    0.6550202348522072, -0.2941787990675794, -0.23153568981902073, 2.253034895825433, -0.35479089535141245,
    -0.20412897125396062, 0.712498847669345, -0.904663136857451, -0.35479089535141245, 4.369703848762114));
  BeginCategory('Eigenvectors degenerate 2,2,2,5,5');
  V := A.Eigenvectors(ev);
  CheckResiduals('deg5', 1e-7);
  offdiag := 0;
  for j := 0 to 4 do
    for k := j+1 to 4 do
    begin
      proj := 0;
      for i := 0 to 4 do proj := proj + V[i,j] * V[i,k].Conjugate;
      if Abs(proj) > offdiag then offdiag := Abs(proj);
    end;
  CmpRAbs('deg5 max |<vi,vj>|', offdiag, 0.0, 1e-7);

  // Rotation 2x2: complex pair, complex eigenvectors
  A.Init(TArrayOfDouble.Create(0,-1, 1,0));
  BeginCategory('Eigenvectors rotation 2x2');
  V := A.Eigenvectors(ev);
  CheckResiduals('rot2', 1e-9);

  // General 4x4 with mixed spectrum
  A.Init(TArrayOfDouble.Create(
    1,-2, 5, 0.5,
    2, 1,-1, 0.25,
    0, 0, 3, 1,
    0, 0, 0,-2));
  BeginCategory('Eigenvectors general 4x4 (mixed spectrum)');
  V := A.Eigenvectors(ev);
  CheckResiduals('gen4', 1e-8);
end;

{ Entry point: runs every test procedure in this unit. }
procedure Run;
begin
  TestTRMatrix;
  TestSolveLinear;
  TestEigenvectors;
  TestComplexEigenvalues;
  TestExceptions;
  TestRealMatrices;
  TestLargeRealInverse;
  TestLargeRealEigen;
  TestLargeRealProducts;
end;

initialization
  RegisterSuite('Real matrix (TRMatrix)', @Run);

end.
