...
begin
  ...
  writeln(MeterPerHourToVerboseString(speed, 5, 0, [pKilo]));
  ...
  if MeterPerSecondToString(speed, 5, 0, []) <> '2 m/s' then halt(1);
  if NewtonToString(force, 5, 0, [])         <> '40 N'  then halt(2);
  ...
  force    := pressure*area;
  area     := force/pressure;
  if PascalToString(pressure)  <> '2 Pa' then halt(1);
  if NewtonToString(force)     <> '10 N' then halt(2);
  if SquareMeterToString(area) <> '5 m2' then halt(3);
  ...
  distance := 5*m;
  work     := force*distance;
  if JouleToString(work, 5, 0, [pDeca])        <> '5 daJ'        then halt(1);
  if JouleToVerboseString(work, 5, 0, [pDeca]) <> '5 decajoules' then halt(2);
  ...
  time  := 10*s;
  power := work/time;
  if WattToString(power, 5, 0, []) <> '5 W' then halt(1);
  ...
  if WattToString(power, 5, 0, []) <> '20 W' then halt(1);
  ...
  if VoltToString(potential) <> '2 V'  then halt(1);
  if CoulombToString(charge) <> '25 C' then halt(2);
  if JouleToString(work)     <> '50 J' then halt(3);
  ...
  if VoltToString(potential) <> '2 V'  then halt(1);
  if AmpereToString(current) <> '5 A'  then halt(2);
  if WattToString(power)     <> '10 W' then halt(3);
  ...
  if VoltToString(potential) <> '50 V' then halt(1);
  ...
  if AmpereToString(current) <> '20 A' then halt(1);
  ...
  if FaradToString(capacitance) <> '0.5 F' then halt(1);
  ...
  if FaradToVerboseString(capacitance) <> '2 farads' then halt(1);
  ...
  time      := 10*s;
  flux      := potential*time;
  if WeberToVerboseString(flux) <> '50 webers' then halt(1);
  ...
  if TeslaToVerboseString(fluxdensity) <> '2.5 teslas' then halt(1);
  ...
  if HenryToString(inductance)        <> '3 H'       then halt(1);
  if HenryToVerboseString(inductance) <> '3 henries' then halt(2);
  writeln('* TEST-21: PASSED');

  // TEST-22 - SIEMENS
  resistance  := 2*Ohm;
  conductance := 1/resistance;
  if SiemensToVerboseString(conductance) <> '0.5 siemens' then halt(1);
  ...
  time    := 5*s;
  charge  := current*time;
  if CoulombToVerboseString(charge) <> '25 coulombs' then halt(1);
  if CoulombToString(charge)        <> '25 C'        then halt(2);
  ...
  if LumenToString(luminousflux) <> '900 lm' then halt(1);
  ...
  dose1 := 10*j/kg;
  dose2 := 5 *j/kg;
  if SquareMeterPerSquareSecondToString(dose1) <> '10 m2/s2' then halt(1);
  if SquareMeterPerSquareSecondToString(dose2) <> '5 m2/s2'  then halt(2);
  if SievertToString(dose1)                    <> '10 Sv'    then halt(3);
  if GrayToString(dose2)                       <> '5 Gy'     then halt(4);
  ...
  if NewtonPerMeterToString(stiffness) <> '5000 N/m' then halt(1);
  ...
  mass    := density*volume;
  volume  := mass/density;
  if KilogramPerCubicMeterToString(density) <> '2 kg/m3' then halt(1);
  if KilogramToString(mass)                 <> '10 kg'   then halt(2);
  if CubicMeterToString(volume)             <> '5 m3'    then halt(3);
  ...
  if NewtonPerCubicMeterToString(specificw) <> '10 N/m3' then halt(1);
  if NewtonToString(force)                  <> '100 N'   then halt(2);
  if CubicMeterToString(volume)             <> '10 m3'   then halt(3);
  ...
  kA     := 0.05;
  force  := kA*normal;
  if NewtonToString(force) <> '5 N' then halt(1);
  ...
  if NewtonToString(force) <> '1 N' then halt(1);
  ...
  if NewtonToString(force, 4, 2, []) <> '1.885 N' then halt(1);
  ...
  if NewtonToString(force, 4, 2, [])                  <> '0.007197 N'  then halt(1);
  if KilogramPerCubicMeterToString(density, 4, 2, []) <> '1.225 kg/m3' then halt(2);
  ...
  if NewtonToString(force, 4, 2, []) <> '1.981E20 N' then halt(1);
  ...
  if JouleToString(Ug, 4, 2, []) <> '980.7 J' then halt(1);
  ...
  if JouleToString(Uc, 4, 2, []) <> '125 J' then halt(1);
  ...
  if JouleToString(Ue, 4, 2, []) <> '500 J' then halt(1);
  ...
  p2    := p*p;
  Uc    := 0.5*p2/mass;
  if  MeterPerSecondToString(p, 4, 2, [])                            <> '50 kg·m/s'      then halt(1);
  if  SquareKilogramSquareMeterPerSquareSecondToString(p2, 4, 2, []) <> '2500 kg2·m2/s2' then halt(2);
  ...
  impulse := p;
  impulse := force*time;
  if NewtonSecondToString(impulse, 4, 2, [pNone, pMilli]) <> '50 N·ms' then halt(1);
  ...
  if PascalToString(pressure, 4, 2, []) <> '196.1 Pa' then halt(1);
  ...
  if NewtonToString(force, 4, 2, []) <> '2.452 N' then halt(1);
  ...
  if CubicMeterPerSecondToString(flowrate, 4, 2, []) <> '5 m3/s' then halt(1);
  ...
  if PascalToString(pressure, 4, 2, []) <> '62.5 Pa' then halt(1);

  distance := 2*m;
  pressure := density*StandardAccelerationOfGravity*distance;
  if PascalToString(pressure, 4, 2, []) <> '98.07 Pa' then halt(2);
  ...
  if MeterPerSecondToString(speed, 4, 2, []) <> '0.3175 m/s' then halt(1);
  ...
  if MeterToString(deltadist, 4, 2, [pMilli]) <> '12 mm' then halt(1);
  ...
  if JoulePerKelvinToString(heatcapacity, 4, 2, []) <> '75 J/K' then halt(1);
  ...
  if KelvinToString(_tf, 4, 2, []) <> '75 K' then halt(1);
  ...
  if WattToString(power, 4, 2, []) <> '825 W' then halt(1);
  ...
  if JouleToString(Uel, 4, 2, [pMilli]) <> '48.53 mJ' then halt(1);
  ...
  q1    := ElectronCharge;
  force := E*q1;
  if NewtonToString(force, 4, 2, [pYocto]) <> '240.3 yN' then halt(1);
  ...
  if NewtonToString(force, 4, 2, [pMilli]) <> '121.3 mN' then halt(1);
  ...
  if VoltPerMeterToString(E, 4, 2, [pMega, pMilli]) <> '7190 MV/mm' then halt(1);
  ...
  if VoltPerMeterToString(E, 4, 2, [pMega, pMilli]) <> '898.8 MV/mm' then halt(1);
  ...
  if VoltPerMeterToString(E, 4, 2, [pGiga, pMilli]) <> '564.7 GV/mm' then halt(1);
  ...
  if AmpereToString(current, 4, 2, [pMilli]) <> '2452 mA' then halt(1);
  ...
  {$ENDIF}
  {$IFDEF UNIX}
  if TeslaToVerboseString(B, 4, 2, [pMicro]) <> '1.2 microteslas' then halt(1);
  {$ENDIF}
  ...
  if TeslaToVerboseString(B, 4, 2, []) <> '2.011 teslas' then halt(1);
  ...
  if NewtonToVerboseString(force, 4, 2, [pMicro]) <> '18.75 micronewtons' then halt(1);
  ...
  if WeberToVerboseString(magneticflux, 4, 2, [pMicro]) <> '1368 microwebers' then halt(1);
  ...
  if VoltToVerboseString(potential, 4, 2, [pMicro]) <> '600 microvolts' then halt(1);
  ...
  if AmpereToVerboseString(current, 4, 2, [pMicro]) <> '5313 microamperes' then halt(1);
  ...
  if MeterToString(wavelen, 4, 2, [pMilli])       <> '-1648 mm'   then halt(1);
  if MeterPerSecondToString(yspeed, 4, 2, [])     <> '-90.69 m/s' then halt(2);
  if MeterPerSquareSecondToString(yacc, 4, 2, []) <> '-7255 m/s2' then halt(3);
  if WattToString(power, 4, 2, [pMilli])          <> '128 mW'     then halt(4);
  writeln('* TEST-61: PASSED');

  // TEST-62 - RELATIVTY: ENERGY
  mass       := 1*kg;
  energy     := mass*SquarePower(SpeedOfLight);
  if ElectronvoltToString(energy, 4, 2, [pTera]) <> '5.61E23 TeV' then halt(1);
  if JouleToString(energy, 4, 2, [pTera])        <> '8.988E4 TJ'  then halt(2);
  ...
  if KilogramMeterPerSecondToString(p, 4, 2, [pPico, pPico, pNone]) <> '2733 pg·pm/s' then halt(1);
  if ElectronvoltToString(energy, 4, 2, [pKilo])                    <> '511 keV'      then halt(2);
  ...
  speed  := len*freq;
  if ElectronvoltToString(energy, 4, 2, [])                         <> '1.24 eV'        then halt(1);
  if KilogramMeterPerSecondToString(p, 4, 2, [pPico, pPico, pNone]) <> '0.6626 pg·pm/s' then halt(2);
  if MeterPerSecondToString(speed, 9, 2, [pPico, pNone])            <> '1000000 pm/s'   then halt(3);
  ...
  if DayToString(time, 5, 0, []) <> '365.2 d' then halt(1);
  ...
  if MeterPerSquareSecondToString(acc, 3, 2, []) <> '9.81 m/s2' then halt(1);
  ...
  if RadianPerSecondToString(omega, 3, 2, []) <> '3.16 rad/s' then halt(1);
  ...
  if SecondToString(time, 4, 2, []) <> '14.19 s' then halt(1);
  writeln('* TEST-69: PASSED');

  // TESTS FROM 70 TO 93
  if KilogramToString(1.0*mg, 10, 10, [pKilo])              <> '1E-6 kg'                   then halt(01);  writeln('* TEST-70: PASSED');
  if SquareKilogramToString(1.0*mg2, 10, 0,  [pKilo])       <> '1E-12 kg2'                 then halt(02);  writeln('* TEST-71: PASSED');
  if KilogramToString(1.0*mg, 10, 10, [pMega])              <> '1E-9 Mg'                   then halt(03);  writeln('* TEST-72: PASSED');
  if SquareKilogramToString(1.0*mg2, 10, 0,  [pMega])       <> '1E-18 Mg2'                 then halt(04);  writeln('* TEST-73: PASSED');
  if KilogramToString(1.0*kg, 10, 0, [pNone])               <> '1000 g'                    then halt(05);  writeln('* TEST-74: PASSED');
  if KilogramToString(1.0*kg, 10, 0, [pKilo])               <> '1 kg'                      then halt(06);  writeln('* TEST-75: PASSED');
  if SquareKilogramToString(1.0*kg2, 10, 0, [pNone])        <> '1000000 g2'                then halt(07);  writeln('* TEST-76: PASSED');
  if SquareKilogramToString(1.0*kg2, 10, 0, [pKilo])        <> '1 kg2'                     then halt(08);  writeln('* TEST-77: PASSED');
  if MeterToString(1.0*km, 10, 0, [])                       <> '1000 m'                    then halt(09);  writeln('* TEST-78: PASSED');
  if SquareMeterToString(1.0*km2, 10, 0, [])                <> '1000000 m2'                then halt(10);  writeln('* TEST-79: PASSED');
  if KilogramToVerboseString(1.0*kg, 10, 0, [pNone])        <> '1000 grams'                then halt(11);  writeln('* TEST-80: PASSED');
  if SquareKilogramToVerboseString(1.0*kg2, 10, 0, [pNone]) <> '1000000 square grams'      then halt(12);  writeln('* TEST-81: PASSED');
  if MeterToVerboseString(1.0*km, 10, 0, [])                <> '1000 meters'               then halt(13);  writeln('* TEST-82: PASSED');
  if SquareMeterToVerboseString(1.0*km2, 10, 0, [])         <> '1000000 square meters'     then halt(14);  writeln('* TEST-83: PASSED');
  if CubicMeterToVerboseString(1.0*km3, 10, 0, [])          <> '1000000000 cubic meters'   then halt(15);  writeln('* TEST-84: PASSED');
  if QuarticMeterToVerboseString(1.0*km4, 10, 0, [])        <> '1E12 quartic meters'       then halt(16);  writeln('* TEST-85: PASSED');
  if SecondToVerboseString(1.0*day, 10, 0, [])              <> '86400 seconds'             then halt(16);  writeln('* TEST-86: PASSED');
  if SecondToVerboseString(1.0*hr, 10, 0, [])               <> '3600 seconds'              then halt(17);  writeln('* TEST-87: PASSED');
  if SecondToString(1.0*day, 10, 0, [])                     <> '86400 s'                   then halt(18);  writeln('* TEST-88: PASSED');
  if SecondToString(1.0*hr, 10, 0, [])                      <> '3600 s'                    then halt(19);  writeln('* TEST-89: PASSED');
  if SquareSecondToVerboseString(1.0*day2, 10, 0, [])       <> '7464960000 square seconds' then halt(20);  writeln('* TEST-90: PASSED');
  if SquareSecondToVerboseString(1.0*hr2, 10, 0, [])        <> '12960000 square seconds'   then halt(21);  writeln('* TEST-91: PASSED');
  if SquareSecondToString(1.0*day2, 10, 0, [])              <> '7464960000 s2'             then halt(22);  writeln('* TEST-92: PASSED');
  if SquareSecondToString(1.0*hr2, 10, 0, [])               <> '12960000 s2'               then halt(23);  writeln('* TEST-93: PASSED');

  // TEST-94
  if WattHourToString(1.0*J, 4, 0, [pMilli])   <> '0.2778 mW·h' then halt(1);
  if AmpereHourToString(1.0*C, 4, 0, [pMilli]) <> '0.2778 mA·h' then halt(2);
  ...
  omega := freq;
  freq  := omega;
  if HertzToString(omega)          <> '10 Hz'    then halt(1);
  if RadianPerSecondToString(freq) <> '10 rad/s' then halt(2);
  writeln('* TEST-95: PASSED');

  // TEST-96
  distance  := 10.5*mm;
  tolerance := 0.2*mm;
  if Utf8ToAnsi(MeterToString(distance, tolerance, 5, 5, [pMilli]))        <> Utf8ToAnsi('10.5 ± 0.2 mm')          then halt(1);
  if Utf8ToAnsi(MeterToVerboseString(distance, tolerance, 5, 5, [pMilli])) <> Utf8ToAnsi('10.5 ± 0.2 millimeters') then halt(2);
  ...
  if MeterToString(radius, 4, 4, [])          <> '5.292E-11 m' then halt(2);
  if MeterPerSecondToString(speed, 4, 4, [])  <> '2.188E6 m/s' then halt(3);
  if ElectronvoltToString(energy, 3, 3, [])   <> '-13.6 eV'    then halt(4);
  if RydbergToString(energy, 3, 3, [])        <> '-1 Ry'       then halt(5);
  ...
  if ('|Ψ| = ' + SquareRootMeterToString(SquareRoot(2/BoxLen)*Sin(1*pi/BoxLen*BoxLen/2), 4, 4, [])) <> '|Ψ| = 6.325 1/√m' then halt(1);
  if ('|Ψ| = ' + SquareRootMeterToString(SquareRoot(2/BoxLen)*Sin(2*pi/BoxLen*BoxLen/4), 4, 4, [])) <> '|Ψ| = 6.325 1/√m' then halt(2);
  if ('|Ψ| = ' + SquareRootMeterToString(SquareRoot(2/BoxLen)*Sin(3*pi/BoxLen*BoxLen/6), 4, 4, [])) <> '|Ψ| = 6.325 1/√m' then halt(3);
  if ('|Ψ| = ' + SquareRootMeterToString(SquareRoot(2/BoxLen)*Sin(4*pi/BoxLen*BoxLen/8), 4, 4, [])) <> '|Ψ| = 6.325 1/√m' then halt(4);

  if ('|Ψ²| = ' + ReciprocalMeterToString(SquarePsi[1])) <> '|Ψ²| = 40 1/m' then halt(5);
  if ('|Ψ²| = ' + ReciprocalMeterToString(SquarePsi[2])) <> '|Ψ²| = 40 1/m' then halt(6);
  if ('|Ψ²| = ' + ReciprocalMeterToString(SquarePsi[3])) <> '|Ψ²| = 40 1/m' then halt(7);
  if ('|Ψ²| = ' + ReciprocalMeterToString(SquarePsi[4])) <> '|Ψ²| = 40 1/m' then halt(8);
  ...
  if MeterToString(radius1, 5, 5, [pKilo]) <> '2.9532 km'  then halt(1);
  if MeterToString(radius2, 5, 5, [])      <> '1.269E10 m' then halt(2);
  ...
  if MeterToString(bfactor, 3, 3, [pNano])         <> '8.87 1/nm' then halt(1);
  ...
  if MeterToString(bfactor, 3, 3, [pNano])         <> '5.12 1/nm' then halt(1);
